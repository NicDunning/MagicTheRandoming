tsrCommon = {
  "Amrou Scout": {
    "name": "Amrou Scout",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Kithkin Rebel Scout",
    "rarity": "Common",
    "abilitiesText": [
      "4, T: Search your library for a Rebel permanent card with converted mana cost 3 or less and put it onto the battlefield. Then shuffle your library.",
      ""
    ],
    "textbox": [
      "4, T: Search your library for a Rebel permanent card with converted mana cost 3 or less and put it onto the battlefield. Then shuffle your library."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Amrou Seekers": {
    "name": "Amrou Seekers",
    "color": "White",
    "cost": "2W",
    "type": "Creature \u2014 Kithkin Rebel",
    "rarity": "Common",
    "abilitiesText": [
      "This card can\u2019t be blocked except by artifact creatures and/or white creatures.",
      ""
    ],
    "textbox": [
      "Amrou Seekers can\u2019t be blocked except by artifact creatures and/or white creatures."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Aven Riftwatcher": {
    "name": "Aven Riftwatcher",
    "color": "White",
    "cost": "2W",
    "type": "Creature \u2014 Bird Rebel Soldier",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield or leaves the battlefield, you gain 2 life.",
      ""
    ],
    "textbox": [
      "Flying",
      "Vanishing 3 (This creature enters the battlefield with three time counters on it. At the beginning of your upkeep, remove a time counter from it. When the last is removed, sacrifice it.)",
      "When Aven Riftwatcher enters the battlefield or leaves the battlefield, you gain 2 life."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 3,
    "abilities": 3
  },
  "Battering Sliver": {
    "name": "Battering Sliver",
    "color": "Red",
    "cost": "5R",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "All Sliver creatures have trample.",
      ""
    ],
    "textbox": [
      "All Sliver creatures have trample."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 6,
    "abilities": 1
  },
  "Benalish Cavalry": {
    "name": "Benalish Cavalry",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Human Knight",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flanking (Whenever a creature without flanking blocks this creature, the blocking creature gets -1/-1 until end of turn.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Blade of the Sixth Pride": {
    "name": "Blade of the Sixth Pride",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Cat Rebel",
    "rarity": "Common",
    "abilitiesText": [
      "",
      ""
    ],
    "textbox": [
      ""
    ],
    "power": "3",
    "toughness": "1",
    "converted": 2,
    "abilities": 0
  },
  "Blightspeaker": {
    "name": "Blightspeaker",
    "color": "Black",
    "cost": "1B",
    "type": "Creature \u2014 Human Rebel Cleric",
    "rarity": "Common",
    "abilitiesText": [
      "T: Target player loses 1 life.",
      "4, T: Search your library for a Rebel permanent card with converted mana cost 3 or less and put it onto the battlefield. Then shuffle your library.",
      ""
    ],
    "textbox": [
      "T: Target player loses 1 life.",
      "4, T: Search your library for a Rebel permanent card with converted mana cost 3 or less and put it onto the battlefield. Then shuffle your library."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Bonesplitter Sliver": {
    "name": "Bonesplitter Sliver",
    "color": "Red",
    "cost": "3R",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "All Sliver creatures get +2/+0.",
      ""
    ],
    "textbox": [
      "All Sliver creatures get +2/+0."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 1
  },
  "Castle Raptors": {
    "name": "Castle Raptors",
    "color": "White",
    "cost": "4W",
    "type": "Creature \u2014 Bird Soldier",
    "rarity": "Common",
    "abilitiesText": [
      "As long as This card is untapped, it gets +0/+2.",
      ""
    ],
    "textbox": [
      "Flying",
      "As long as Castle Raptors is untapped, it gets +0/+2."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 5,
    "abilities": 2
  },
  "Children of Korlis": {
    "name": "Children of Korlis",
    "color": "White",
    "cost": "W",
    "type": "Creature \u2014 Human Rebel Cleric",
    "rarity": "Common",
    "abilitiesText": [
      "Sacrifice This card: You gain life equal to the life you\u2019ve lost this turn. (Damage causes loss of life.)",
      ""
    ],
    "textbox": [
      "Sacrifice Children of Korlis: You gain life equal to the life you\u2019ve lost this turn. (Damage causes loss of life.)"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 1
  },
  "Citanul Woodreaders": {
    "name": "Citanul Woodreaders",
    "color": "Green",
    "cost": "2G",
    "type": "Creature \u2014 Human Druid",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, if it was kicked, draw two cards.",
      ""
    ],
    "textbox": [
      "Kicker 2G (You may pay an additional 2G as you cast this spell.)",
      "When Citanul Woodreaders enters the battlefield, if it was kicked, draw two cards."
    ],
    "power": "1",
    "toughness": "4",
    "converted": 3,
    "abilities": 2
  },
  "Coal Stoker": {
    "name": "Coal Stoker",
    "color": "Red",
    "cost": "3R",
    "type": "Creature \u2014 Elemental",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, if you cast it from your hand, add RRR.",
      ""
    ],
    "textbox": [
      "When Coal Stoker enters the battlefield, if you cast it from your hand, add RRR."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 4,
    "abilities": 1
  },
  "Coral Trickster": {
    "name": "Coral Trickster",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Merfolk Rogue",
    "rarity": "Common",
    "abilitiesText": [
      "When This card is turned face up, you may tap or untap target permanent.",
      ""
    ],
    "textbox": [
      "Morph U (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)",
      "When Coral Trickster is turned face up, you may tap or untap target permanent."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Corpulent Corpse": {
    "name": "Corpulent Corpse",
    "color": "Black",
    "cost": "5B",
    "type": "Creature \u2014 Zombie",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Fear (This creature can\u2019t be blocked except by artifact creatures and/or black creatures.)",
      "Suspend 5\u2014B (Rather than cast this card from your hand, you may pay B and exile it with five time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "3",
    "toughness": "3",
    "converted": 6,
    "abilities": 2
  },
  "Crookclaw Transmuter": {
    "name": "Crookclaw Transmuter",
    "color": "Blue",
    "cost": "3U",
    "type": "Creature \u2014 Bird Wizard",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, switch target creature\u2019s power and toughness until end of turn.",
      ""
    ],
    "textbox": [
      "Flash (You may cast this spell any time you could cast an instant.)",
      "Flying",
      "When Crookclaw Transmuter enters the battlefield, switch target creature\u2019s power and toughness until end of turn."
    ],
    "power": "3",
    "toughness": "1",
    "converted": 4,
    "abilities": 3
  },
  "Cutthroat il-Dal": {
    "name": "Cutthroat il-Dal",
    "color": "Black",
    "cost": "3B",
    "type": "Creature \u2014 Human Rogue",
    "rarity": "Common",
    "abilitiesText": [
      "Hellbent \u2014 This card has shadow as long as you have no cards in hand. (It can block or be blocked by only creatures with shadow.)",
      ""
    ],
    "textbox": [
      "Hellbent \u2014 Cutthroat il-Dal has shadow as long as you have no cards in hand. (It can block or be blocked by only creatures with shadow.)"
    ],
    "power": "4",
    "toughness": "1",
    "converted": 4,
    "abilities": 1
  },
  "Deadly Grub": {
    "name": "Deadly Grub",
    "color": "Black",
    "cost": "2B",
    "type": "Creature \u2014 Insect",
    "rarity": "Common",
    "abilitiesText": [
      "When This card dies, if it had no time counters on it, create a 6/1 green Insect creature token with shroud. (It can\u2019t be the target of spells or abilities.)",
      ""
    ],
    "textbox": [
      "Vanishing 3 (This creature enters the battlefield with three time counters on it. At the beginning of your upkeep, remove a time counter from it. When the last is removed, sacrifice it.)",
      "When Deadly Grub dies, if it had no time counters on it, create a 6/1 green Insect creature token with shroud. (It can\u2019t be the target of spells or abilities.)"
    ],
    "power": "3",
    "toughness": "1",
    "converted": 3,
    "abilities": 2
  },
  "Deathspore Thallid": {
    "name": "Deathspore Thallid",
    "color": "Black",
    "cost": "1B",
    "type": "Creature \u2014 Zombie Fungus",
    "rarity": "Common",
    "abilitiesText": [
      "At the beginning of your upkeep, put a spore counter on This card.",
      "Remove three spore counters from This card: Create a 1/1 green Saproling creature token.",
      "Sacrifice a Saproling: Target creature gets -1/-1 until end of turn.",
      ""
    ],
    "textbox": [
      "At the beginning of your upkeep, put a spore counter on Deathspore Thallid.",
      "Remove three spore counters from Deathspore Thallid: Create a 1/1 green Saproling creature token.",
      "Sacrifice a Saproling: Target creature gets -1/-1 until end of turn."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 3
  },
  "Deepcavern Imp": {
    "name": "Deepcavern Imp",
    "color": "Black",
    "cost": "2B",
    "type": "Creature \u2014 Imp Rebel",
    "rarity": "Common",
    "abilitiesText": [
      "Echo\u2014Discard a card. (At the beginning of your upkeep, if this came under your control since the beginning of your last upkeep, sacrifice it unless you pay its echo cost.)",
      ""
    ],
    "textbox": [
      "Flying, haste",
      "Echo\u2014Discard a card. (At the beginning of your upkeep, if this came under your control since the beginning of your last upkeep, sacrifice it unless you pay its echo cost.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 3
  },
  "Dreamscape Artist": {
    "name": "Dreamscape Artist",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Human Spellshaper",
    "rarity": "Common",
    "abilitiesText": [
      "2U, T, Discard a card, Sacrifice a land: Search your library for up to two basic land cards, put them onto the battlefield, then shuffle your library.",
      ""
    ],
    "textbox": [
      "2U, T, Discard a card, Sacrifice a land: Search your library for up to two basic land cards, put them onto the battlefield, then shuffle your library."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Dream Stalker": {
    "name": "Dream Stalker",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Illusion",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, return a permanent you control to its owner\u2019s hand.",
      ""
    ],
    "textbox": [
      "When Dream Stalker enters the battlefield, return a permanent you control to its owner\u2019s hand."
    ],
    "power": "1",
    "toughness": "5",
    "converted": 2,
    "abilities": 1
  },
  "Drifter il-Dal": {
    "name": "Drifter il-Dal",
    "color": "Blue",
    "cost": "U",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Common",
    "abilitiesText": [
      "At the beginning of your upkeep, sacrifice This card unless you pay U.",
      ""
    ],
    "textbox": [
      "Shadow (This creature can block or be blocked by only creatures with shadow.)",
      "At the beginning of your upkeep, sacrifice Drifter il-Dal unless you pay U."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 1,
    "abilities": 2
  },
  "Durkwood Baloth": {
    "name": "Durkwood Baloth",
    "color": "Green",
    "cost": "4GG",
    "type": "Creature \u2014 Beast",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Suspend 5\u2014G (Rather than cast this card from your hand, you may pay G and exile it with five time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "5",
    "toughness": "5",
    "converted": 6,
    "abilities": 1
  },
  "Errant Doomsayers": {
    "name": "Errant Doomsayers",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Human Rebel",
    "rarity": "Common",
    "abilitiesText": [
      "T: Tap target creature with toughness 2 or less.",
      ""
    ],
    "textbox": [
      "T: Tap target creature with toughness 2 or less."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Errant Ephemeron": {
    "name": "Errant Ephemeron",
    "color": "Blue",
    "cost": "6U",
    "type": "Creature \u2014 Illusion",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying",
      "Suspend 4\u20141U (Rather than cast this card from your hand, you may pay 1U and exile it with four time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "4",
    "toughness": "4",
    "converted": 7,
    "abilities": 2
  },
  "Fathom Seer": {
    "name": "Fathom Seer",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Illusion",
    "rarity": "Common",
    "abilitiesText": [
      "Morph\u2014Return two Islands you control to their owner\u2019s hand. (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)",
      "When This card is turned face up, draw two cards.",
      ""
    ],
    "textbox": [
      "Morph\u2014Return two Islands you control to their owner\u2019s hand. (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)",
      "When Fathom Seer is turned face up, draw two cards."
    ],
    "power": "1",
    "toughness": "3",
    "converted": 2,
    "abilities": 2
  },
  "Gathan Raiders": {
    "name": "Gathan Raiders",
    "color": "Red",
    "cost": "3RR",
    "type": "Creature \u2014 Human Warrior",
    "rarity": "Common",
    "abilitiesText": [
      "Hellbent \u2014 This card gets +2/+2 as long as you have no cards in hand.",
      "Morph\u2014Discard a card. (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)",
      ""
    ],
    "textbox": [
      "Hellbent \u2014 Gathan Raiders gets +2/+2 as long as you have no cards in hand.",
      "Morph\u2014Discard a card. (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)"
    ],
    "power": "3",
    "toughness": "3",
    "converted": 5,
    "abilities": 2
  },
  "Gemhide Sliver": {
    "name": "Gemhide Sliver",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "All Slivers have \u201cT: Add one mana of any color.\u201d",
      ""
    ],
    "textbox": [
      "All Slivers have \u201cT: Add one mana of any color.\u201d"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Giant Dustwasp": {
    "name": "Giant Dustwasp",
    "color": "Green",
    "cost": "3GG",
    "type": "Creature \u2014 Insect",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying",
      "Suspend 4\u20141G (Rather than cast this card from your hand, you may pay 1G and exile it with four time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "3",
    "toughness": "3",
    "converted": 5,
    "abilities": 2
  },
  "Gorgon Recluse": {
    "name": "Gorgon Recluse",
    "color": "Black",
    "cost": "3BB",
    "type": "Creature \u2014 Gorgon",
    "rarity": "Common",
    "abilitiesText": [
      "Whenever This card blocks or becomes blocked by a nonblack creature, destroy that creature at end of combat.",
      ""
    ],
    "textbox": [
      "Whenever Gorgon Recluse blocks or becomes blocked by a nonblack creature, destroy that creature at end of combat.",
      "Madness BB (If you discard this card, discard it into exile. When you do, cast it for its madness cost or put it into your graveyard.)"
    ],
    "power": "2",
    "toughness": "4",
    "converted": 5,
    "abilities": 2
  },
  "Gossamer Phantasm": {
    "name": "Gossamer Phantasm",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Illusion",
    "rarity": "Common",
    "abilitiesText": [
      "When This card becomes the target of a spell or ability, sacrifice it.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Gossamer Phantasm becomes the target of a spell or ability, sacrifice it."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Grave Scrabbler": {
    "name": "Grave Scrabbler",
    "color": "Black",
    "cost": "3B",
    "type": "Creature \u2014 Zombie",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, if its madness cost was paid, you may return target creature card from a graveyard to its owner\u2019s hand.",
      ""
    ],
    "textbox": [
      "Madness 1B (If you discard this card, discard it into exile. When you do, cast it for its madness cost or put it into your graveyard.)",
      "When Grave Scrabbler enters the battlefield, if its madness cost was paid, you may return target creature card from a graveyard to its owner\u2019s hand."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 2
  },
  "Greenseeker": {
    "name": "Greenseeker",
    "color": "Green",
    "cost": "G",
    "type": "Creature \u2014 Elf Spellshaper",
    "rarity": "Common",
    "abilitiesText": [
      "G, T, Discard a card: Search your library for a basic land card, reveal it, and put it into your hand. Then shuffle your library.",
      ""
    ],
    "textbox": [
      "G, T, Discard a card: Search your library for a basic land card, reveal it, and put it into your hand. Then shuffle your library."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 1
  },
  "Grinning Ignus": {
    "name": "Grinning Ignus",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Elemental",
    "rarity": "Common",
    "abilitiesText": [
      "R, Return This card to its owner\u2019s hand: Add CCR. Activate this ability only any time you could cast a sorcery.",
      ""
    ],
    "textbox": [
      "R, Return Grinning Ignus to its owner\u2019s hand: Add CCR. Activate this ability only any time you could cast a sorcery."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Homing Sliver": {
    "name": "Homing Sliver",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "Each Sliver card in each player\u2019s hand has slivercycling 3.",
      "Slivercycling 3 (3, Discard this card: Search your library for a Sliver card, reveal it, put it into your hand, then shuffle your library.)",
      ""
    ],
    "textbox": [
      "Each Sliver card in each player\u2019s hand has slivercycling 3.",
      "Slivercycling 3 (3, Discard this card: Search your library for a Sliver card, reveal it, put it into your hand, then shuffle your library.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 2
  },
  "Infiltrator il-Kor": {
    "name": "Infiltrator il-Kor",
    "color": "Blue",
    "cost": "4U",
    "type": "Creature \u2014 Kor Rogue",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Shadow (This creature can block or be blocked by only creatures with shadow.)",
      "Suspend 2\u20141U (Rather than cast this card from your hand, you may pay 1U and exile it with two time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "3",
    "toughness": "1",
    "converted": 5,
    "abilities": 2
  },
  "Ivory Giant": {
    "name": "Ivory Giant",
    "color": "White",
    "cost": "5WW",
    "type": "Creature \u2014 Giant",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, tap all nonwhite creatures.",
      ""
    ],
    "textbox": [
      "When Ivory Giant enters the battlefield, tap all nonwhite creatures.",
      "Suspend 5\u2014W (Rather than cast this card from your hand, you may pay W and exile it with five time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "3",
    "toughness": "4",
    "converted": 7,
    "abilities": 2
  },
  "Jhoira's Timebug": {
    "name": "Jhoira's Timebug",
    "color": "Artifact",
    "cost": "2",
    "type": "Artifact Creature \u2014 Insect",
    "rarity": "Common",
    "abilitiesText": [
      "T: Choose target permanent you control or suspended card you own. If that permanent or card has a time counter on it, you may remove a time counter from it or put another time counter on it.",
      ""
    ],
    "textbox": [
      "T: Choose target permanent you control or suspended card you own. If that permanent or card has a time counter on it, you may remove a time counter from it or put another time counter on it."
    ],
    "power": "1",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Kavu Primarch": {
    "name": "Kavu Primarch",
    "color": "Green",
    "cost": "3G",
    "type": "Creature \u2014 Kavu",
    "rarity": "Common",
    "abilitiesText": [
      "If This card was kicked, it enters the battlefield with four +1/+1 counters on it.",
      ""
    ],
    "textbox": [
      "Kicker 4 (You may pay an additional 4 as you cast this spell.)",
      "Convoke (Your creatures can help cast this spell. Each creature you tap while casting this spell pays for 1 or one mana of that creature\u2019s color.)",
      "If Kavu Primarch was kicked, it enters the battlefield with four +1/+1 counters on it."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 4,
    "abilities": 3
  },
  "Keldon Halberdier": {
    "name": "Keldon Halberdier",
    "color": "Red",
    "cost": "4R",
    "type": "Creature \u2014 Human Warrior",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "First strike",
      "Suspend 4\u2014R (Rather than cast this card from your hand, you may pay R and exile it with four time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "4",
    "toughness": "1",
    "converted": 5,
    "abilities": 2
  },
  "Knight of Sursi": {
    "name": "Knight of Sursi",
    "color": "White",
    "cost": "3W",
    "type": "Creature \u2014 Human Knight",
    "rarity": "Common",
    "abilitiesText": [
      "Flying; flanking (Whenever a creature without flanking blocks this creature, the blocking creature gets -1/-1 until end of turn.)",
      ""
    ],
    "textbox": [
      "Flying; flanking (Whenever a creature without flanking blocks this creature, the blocking creature gets -1/-1 until end of turn.)",
      "Suspend 3\u2014W (Rather than cast this card from your hand, you may pay W and exile it with three time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 2
  },
  "Looter il-Kor": {
    "name": "Looter il-Kor",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Kor Rogue",
    "rarity": "Common",
    "abilitiesText": [
      "Whenever This card deals damage to an opponent, draw a card, then discard a card.",
      ""
    ],
    "textbox": [
      "Shadow (This creature can block or be blocked by only creatures with shadow.)",
      "Whenever Looter il-Kor deals damage to an opponent, draw a card, then discard a card."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Lymph Sliver": {
    "name": "Lymph Sliver",
    "color": "White",
    "cost": "4W",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "All Sliver creatures have absorb 1. (If a source would deal damage to a Sliver, prevent 1 of that damage.)",
      ""
    ],
    "textbox": [
      "All Sliver creatures have absorb 1. (If a source would deal damage to a Sliver, prevent 1 of that damage.)"
    ],
    "power": "3",
    "toughness": "3",
    "converted": 5,
    "abilities": 1
  },
  "Mass of Ghouls": {
    "name": "Mass of Ghouls",
    "color": "Black",
    "cost": "3BB",
    "type": "Creature \u2014 Zombie Warrior",
    "rarity": "Common",
    "abilitiesText": [
      "",
      ""
    ],
    "textbox": [
      ""
    ],
    "power": "5",
    "toughness": "3",
    "converted": 5,
    "abilities": 0
  },
  "Mogg War Marshal": {
    "name": "Mogg War Marshal",
    "color": "Red",
    "cost": "1R",
    "type": "Creature \u2014 Goblin Warrior",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield or dies, create a 1/1 red Goblin creature token.",
      ""
    ],
    "textbox": [
      "Echo 1R (At the beginning of your upkeep, if this came under your control since the beginning of your last upkeep, sacrifice it unless you pay its echo cost.)",
      "When Mogg War Marshal enters the battlefield or dies, create a 1/1 red Goblin creature token."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Nantuko Shaman": {
    "name": "Nantuko Shaman",
    "color": "Green",
    "cost": "2G",
    "type": "Creature \u2014 Insect Shaman",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, if you control no tapped lands, draw a card.",
      ""
    ],
    "textbox": [
      "When Nantuko Shaman enters the battlefield, if you control no tapped lands, draw a card.",
      "Suspend 1\u20142GG (Rather than cast this card from your hand, you may pay 2GG and exile it with a time counter on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "3",
    "toughness": "2",
    "converted": 3,
    "abilities": 2
  },
  "Needlepeak Spider": {
    "name": "Needlepeak Spider",
    "color": "Red",
    "cost": "3R",
    "type": "Creature \u2014 Spider",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Reach (This creature can block creatures with flying.)"
    ],
    "power": "4",
    "toughness": "2",
    "converted": 4,
    "abilities": 1
  },
  "Pallid Mycoderm": {
    "name": "Pallid Mycoderm",
    "color": "White",
    "cost": "3W",
    "type": "Creature \u2014 Fungus",
    "rarity": "Common",
    "abilitiesText": [
      "At the beginning of your upkeep, put a spore counter on This card.",
      "Remove three spore counters from This card: Create a 1/1 green Saproling creature token.",
      "Sacrifice a Saproling: Each creature you control that\u2019s a Fungus or a Saproling gets +1/+1 until end of turn.",
      ""
    ],
    "textbox": [
      "At the beginning of your upkeep, put a spore counter on Pallid Mycoderm.",
      "Remove three spore counters from Pallid Mycoderm: Create a 1/1 green Saproling creature token.",
      "Sacrifice a Saproling: Each creature you control that\u2019s a Fungus or a Saproling gets +1/+1 until end of turn."
    ],
    "power": "2",
    "toughness": "4",
    "converted": 4,
    "abilities": 3
  },
  "Penumbra Spider": {
    "name": "Penumbra Spider",
    "color": "Green",
    "cost": "2GG",
    "type": "Creature \u2014 Spider",
    "rarity": "Common",
    "abilitiesText": [
      "When This card dies, create a 2/4 black Spider creature token with reach.",
      ""
    ],
    "textbox": [
      "Reach",
      "When Penumbra Spider dies, create a 2/4 black Spider creature token with reach."
    ],
    "power": "2",
    "toughness": "4",
    "converted": 4,
    "abilities": 2
  },
  "Pit Keeper": {
    "name": "Pit Keeper",
    "color": "Black",
    "cost": "1B",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, if you have four or more creature cards in your graveyard, you may return target creature card from your graveyard to your hand.",
      ""
    ],
    "textbox": [
      "When Pit Keeper enters the battlefield, if you have four or more creature cards in your graveyard, you may return target creature card from your graveyard to your hand."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Primal Plasma": {
    "name": "Primal Plasma",
    "color": "Blue",
    "cost": "3U",
    "type": "Creature \u2014 Elemental Shapeshifter",
    "rarity": "Common",
    "abilitiesText": [
      "As This card enters the battlefield, it becomes your choice of a 3/3 creature, a 2/2 creature with flying, or a 1/6 creature with defender.",
      ""
    ],
    "textbox": [
      "As Primal Plasma enters the battlefield, it becomes your choice of a 3/3 creature, a 2/2 creature with flying, or a 1/6 creature with defender."
    ],
    "power": "*",
    "toughness": "*",
    "converted": 4,
    "abilities": 1
  },
  "Rathi Trapper": {
    "name": "Rathi Trapper",
    "color": "Black",
    "cost": "1B",
    "type": "Creature \u2014 Human Rebel Rogue",
    "rarity": "Common",
    "abilitiesText": [
      "B, T: Tap target creature.",
      ""
    ],
    "textbox": [
      "B, T: Tap target creature."
    ],
    "power": "1",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Reckless Wurm": {
    "name": "Reckless Wurm",
    "color": "Red",
    "cost": "3RR",
    "type": "Creature \u2014 Wurm",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Trample",
      "Madness 2R (If you discard this card, discard it into exile. When you do, cast it for its madness cost or put it into your graveyard.)"
    ],
    "power": "4",
    "toughness": "4",
    "converted": 5,
    "abilities": 2
  },
  "Reflex Sliver": {
    "name": "Reflex Sliver",
    "color": "Green",
    "cost": "3G",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "All Sliver creatures have haste.",
      ""
    ],
    "textbox": [
      "All Sliver creatures have haste."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 1
  },
  "Ridged Kusite": {
    "name": "Ridged Kusite",
    "color": "Black",
    "cost": "B",
    "type": "Creature \u2014 Horror Spellshaper",
    "rarity": "Common",
    "abilitiesText": [
      "1B, T, Discard a card: Target creature gets +1/+0 and gains first strike until end of turn.",
      ""
    ],
    "textbox": [
      "1B, T, Discard a card: Target creature gets +1/+0 and gains first strike until end of turn."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 1
  },
  "Rift Elemental": {
    "name": "Rift Elemental",
    "color": "Red",
    "cost": "R",
    "type": "Creature \u2014 Elemental",
    "rarity": "Common",
    "abilitiesText": [
      "1R, Remove a time counter from a permanent you control or suspended card you own: This card gets +2/+0 until end of turn.",
      ""
    ],
    "textbox": [
      "1R, Remove a time counter from a permanent you control or suspended card you own: Rift Elemental gets +2/+0 until end of turn."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 1
  },
  "Sangrophage": {
    "name": "Sangrophage",
    "color": "Black",
    "cost": "BB",
    "type": "Creature \u2014 Zombie",
    "rarity": "Common",
    "abilitiesText": [
      "At the beginning of your upkeep, tap This card unless you pay 2 life.",
      ""
    ],
    "textbox": [
      "At the beginning of your upkeep, tap Sangrophage unless you pay 2 life."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 2,
    "abilities": 1
  },
  "Sarcomite Myr": {
    "name": "Sarcomite Myr",
    "color": "Blue",
    "cost": "2U",
    "type": "Artifact Creature \u2014 Myr",
    "rarity": "Common",
    "abilitiesText": [
      "2: This card gains flying until end of turn.",
      "2, Sacrifice This card: Draw a card.",
      ""
    ],
    "textbox": [
      "2: Sarcomite Myr gains flying until end of turn.",
      "2, Sacrifice Sarcomite Myr: Draw a card."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 3,
    "abilities": 2
  },
  "Shade of Trokair": {
    "name": "Shade of Trokair",
    "color": "White",
    "cost": "3W",
    "type": "Creature \u2014 Shade",
    "rarity": "Common",
    "abilitiesText": [
      "W: This card gets +1/+1 until end of turn.",
      ""
    ],
    "textbox": [
      "W: Shade of Trokair gets +1/+1 until end of turn.",
      "Suspend 3\u2014W (Rather than cast this card from your hand, you may pay W and exile it with three time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "1",
    "toughness": "2",
    "converted": 4,
    "abilities": 2
  },
  "Sidewinder Sliver": {
    "name": "Sidewinder Sliver",
    "color": "White",
    "cost": "W",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "All Sliver creatures have flanking. (Whenever a creature without flanking blocks a Sliver, the blocking creature gets -1/-1 until end of turn.)",
      ""
    ],
    "textbox": [
      "All Sliver creatures have flanking. (Whenever a creature without flanking blocks a Sliver, the blocking creature gets -1/-1 until end of turn.)"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 1
  },
  "Simian Spirit Guide": {
    "name": "Simian Spirit Guide",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Ape Spirit",
    "rarity": "Common",
    "abilitiesText": [
      "Exile This card from your hand: Add R.",
      ""
    ],
    "textbox": [
      "Exile Simian Spirit Guide from your hand: Add R."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Sinew Sliver": {
    "name": "Sinew Sliver",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "All Sliver creatures get +1/+1.",
      ""
    ],
    "textbox": [
      "All Sliver creatures get +1/+1."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Skirk Shaman": {
    "name": "Skirk Shaman",
    "color": "Red",
    "cost": "1RR",
    "type": "Creature \u2014 Goblin Shaman",
    "rarity": "Common",
    "abilitiesText": [
      "This card can\u2019t be blocked except by artifact creatures and/or red creatures.",
      ""
    ],
    "textbox": [
      "Skirk Shaman can\u2019t be blocked except by artifact creatures and/or red creatures."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Slipstream Serpent": {
    "name": "Slipstream Serpent",
    "color": "Blue",
    "cost": "7U",
    "type": "Creature \u2014 Serpent",
    "rarity": "Common",
    "abilitiesText": [
      "This card can\u2019t attack unless defending player controls an Island.",
      "When you control no Islands, sacrifice This card.",
      ""
    ],
    "textbox": [
      "Slipstream Serpent can\u2019t attack unless defending player controls an Island.",
      "When you control no Islands, sacrifice Slipstream Serpent.",
      "Morph 5U (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)"
    ],
    "power": "6",
    "toughness": "6",
    "converted": 8,
    "abilities": 3
  },
  "Spiketail Drakeling": {
    "name": "Spiketail Drakeling",
    "color": "Blue",
    "cost": "1UU",
    "type": "Creature \u2014 Drake",
    "rarity": "Common",
    "abilitiesText": [
      "Sacrifice This card: Counter target spell unless its controller pays 2.",
      ""
    ],
    "textbox": [
      "Flying",
      "Sacrifice Spiketail Drakeling: Counter target spell unless its controller pays 2."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 2
  },
  "Spinneret Sliver": {
    "name": "Spinneret Sliver",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "All Sliver creatures have reach. (They can block creatures with flying.)",
      ""
    ],
    "textbox": [
      "All Sliver creatures have reach. (They can block creatures with flying.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Sporoloth Ancient": {
    "name": "Sporoloth Ancient",
    "color": "Green",
    "cost": "3GG",
    "type": "Creature \u2014 Fungus",
    "rarity": "Common",
    "abilitiesText": [
      "At the beginning of your upkeep, put a spore counter on This card.",
      "Creatures you control have \u201cRemove two spore counters from this creature: Create a 1/1 green Saproling creature token.\u201d",
      ""
    ],
    "textbox": [
      "At the beginning of your upkeep, put a spore counter on Sporoloth Ancient.",
      "Creatures you control have \u201cRemove two spore counters from this creature: Create a 1/1 green Saproling creature token.\u201d"
    ],
    "power": "4",
    "toughness": "4",
    "converted": 5,
    "abilities": 2
  },
  "Stingscourger": {
    "name": "Stingscourger",
    "color": "Red",
    "cost": "1R",
    "type": "Creature \u2014 Goblin Warrior",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, return target creature an opponent controls to its owner\u2019s hand.",
      ""
    ],
    "textbox": [
      "Echo 3R (At the beginning of your upkeep, if this came under your control since the beginning of your last upkeep, sacrifice it unless you pay its echo cost.)",
      "When Stingscourger enters the battlefield, return target creature an opponent controls to its owner\u2019s hand."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 2
  },
  "Thallid Germinator": {
    "name": "Thallid Germinator",
    "color": "Green",
    "cost": "2G",
    "type": "Creature \u2014 Fungus",
    "rarity": "Common",
    "abilitiesText": [
      "At the beginning of your upkeep, put a spore counter on This card.",
      "Remove three spore counters from This card: Create a 1/1 green Saproling creature token.",
      "Sacrifice a Saproling: Target creature gets +1/+1 until end of turn.",
      ""
    ],
    "textbox": [
      "At the beginning of your upkeep, put a spore counter on Thallid Germinator.",
      "Remove three spore counters from Thallid Germinator: Create a 1/1 green Saproling creature token.",
      "Sacrifice a Saproling: Target creature gets +1/+1 until end of turn."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 3
  },
  "Thallid Shell-Dweller": {
    "name": "Thallid Shell-Dweller",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Fungus",
    "rarity": "Common",
    "abilitiesText": [
      "At the beginning of your upkeep, put a spore counter on This card.",
      "Remove three spore counters from This card: Create a 1/1 green Saproling creature token.",
      ""
    ],
    "textbox": [
      "Defender",
      "At the beginning of your upkeep, put a spore counter on Thallid Shell-Dweller.",
      "Remove three spore counters from Thallid Shell-Dweller: Create a 1/1 green Saproling creature token."
    ],
    "power": "0",
    "toughness": "5",
    "converted": 2,
    "abilities": 3
  },
  "Thornweald Archer": {
    "name": "Thornweald Archer",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Elf Archer",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Reach (This creature can block creatures with flying.)",
      "Deathtouch (Any amount of damage this deals to a creature is enough to destroy it.)"
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Tolarian Sentinel": {
    "name": "Tolarian Sentinel",
    "color": "Blue",
    "cost": "3U",
    "type": "Creature \u2014 Human Spellshaper",
    "rarity": "Common",
    "abilitiesText": [
      "U, T, Discard a card: Return target permanent you control to its owner\u2019s hand.",
      ""
    ],
    "textbox": [
      "Flying",
      "U, T, Discard a card: Return target permanent you control to its owner\u2019s hand."
    ],
    "power": "1",
    "toughness": "3",
    "converted": 4,
    "abilities": 2
  },
  "Trespasser il-Vec": {
    "name": "Trespasser il-Vec",
    "color": "Black",
    "cost": "2B",
    "type": "Creature \u2014 Human Rogue",
    "rarity": "Common",
    "abilitiesText": [
      "Discard a card: This card gains shadow until end of turn. (It can block or be blocked by only creatures with shadow.)",
      ""
    ],
    "textbox": [
      "Discard a card: Trespasser il-Vec gains shadow until end of turn. (It can block or be blocked by only creatures with shadow.)"
    ],
    "power": "3",
    "toughness": "1",
    "converted": 3,
    "abilities": 1
  },
  "Two-Headed Sliver": {
    "name": "Two-Headed Sliver",
    "color": "Red",
    "cost": "1R",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "All Sliver creatures have menace. (They can\u2019t be blocked except by two or more creatures.)",
      ""
    ],
    "textbox": [
      "All Sliver creatures have menace. (They can\u2019t be blocked except by two or more creatures.)"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Uktabi Drake": {
    "name": "Uktabi Drake",
    "color": "Green",
    "cost": "G",
    "type": "Creature \u2014 Drake",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying, haste",
      "Echo 1GG (At the beginning of your upkeep, if this came under your control since the beginning of your last upkeep, sacrifice it unless you pay its echo cost.)"
    ],
    "power": "2",
    "toughness": "1",
    "converted": 1,
    "abilities": 3
  },
  "Urborg Syphon-Mage": {
    "name": "Urborg Syphon-Mage",
    "color": "Black",
    "cost": "2B",
    "type": "Creature \u2014 Human Spellshaper",
    "rarity": "Common",
    "abilitiesText": [
      "2B, T, Discard a card: Each other player loses 2 life. You gain life equal to the life lost this way.",
      ""
    ],
    "textbox": [
      "2B, T, Discard a card: Each other player loses 2 life. You gain life equal to the life lost this way."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Veiling Oddity": {
    "name": "Veiling Oddity",
    "color": "Blue",
    "cost": "3U",
    "type": "Creature \u2014 Illusion",
    "rarity": "Common",
    "abilitiesText": [
      "When the last time counter is removed from This card while it\u2019s exiled, creatures can\u2019t be blocked this turn.",
      ""
    ],
    "textbox": [
      "Suspend 4\u20141U (Rather than cast this card from your hand, you may pay 1U and exile it with four time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)",
      "When the last time counter is removed from Veiling Oddity while it\u2019s exiled, creatures can\u2019t be blocked this turn."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 4,
    "abilities": 2
  },
  "Virulent Sliver": {
    "name": "Virulent Sliver",
    "color": "Green",
    "cost": "G",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "All Sliver creatures have poisonous 1. (Whenever a Sliver deals combat damage to a player, that player gets a poison counter. A player with ten or more poison counters loses the game.)",
      ""
    ],
    "textbox": [
      "All Sliver creatures have poisonous 1. (Whenever a Sliver deals combat damage to a player, that player gets a poison counter. A player with ten or more poison counters loses the game.)"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 1
  },
  "Watcher Sliver": {
    "name": "Watcher Sliver",
    "color": "White",
    "cost": "3W",
    "type": "Creature \u2014 Sliver",
    "rarity": "Common",
    "abilitiesText": [
      "All Sliver creatures get +0/+2.",
      ""
    ],
    "textbox": [
      "All Sliver creatures get +0/+2."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 1
  },
  "Whitemane Lion": {
    "name": "Whitemane Lion",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Cat",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, return a creature you control to its owner\u2019s hand.",
      ""
    ],
    "textbox": [
      "Flash (You may cast this spell any time you could cast an instant.)",
      "When Whitemane Lion enters the battlefield, return a creature you control to its owner\u2019s hand."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 2
  },
  "keywordsCommon": {
    "abilitiesText": [
      "Flying",
      "Vanishing 3",
      "Flanking",
      "Kicker 2g",
      "Morph u",
      "Fear",
      "Suspend 5\u2014b",
      "Flash",
      "Haste",
      "Shadow",
      "Suspend 5\u2014g",
      "Suspend 4\u20141u",
      "Suspend 4\u20141g",
      "Madness bb",
      "Madness 1b",
      "Suspend 2\u20141u",
      "Suspend 5\u2014w",
      "Kicker 4",
      "Convoke",
      "First strike",
      "Suspend 4\u2014r",
      "Suspend 3\u2014w",
      "Echo 1r",
      "Suspend 1\u20142gg",
      "Reach",
      "Trample",
      "Madness 2r",
      "Morph 5u",
      "Echo 3r",
      "Defender",
      "Deathtouch",
      "Echo 1gg",
    ]
  }
}

tsrUncommon = {
  "Aven Mindcensor": {
    "name": "Aven Mindcensor",
    "color": "White",
    "cost": "2W",
    "type": "Creature \u2014 Bird Wizard",
    "rarity": "Uncommon",
    "abilitiesText": [
      "If an opponent would search a library, that player searches the top four cards of that library instead.",
      ""
    ],
    "textbox": [
      "Flash",
      "Flying",
      "If an opponent would search a library, that player searches the top four cards of that library instead."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 3,
    "abilities": 3
  },
  "Basalt Gargoyle": {
    "name": "Basalt Gargoyle",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Gargoyle",
    "rarity": "Uncommon",
    "abilitiesText": [
      "R: This card gets +0/+1 until end of turn.",
      ""
    ],
    "textbox": [
      "Flying",
      "Echo 2R (At the beginning of your upkeep, if this came under your control since the beginning of your last upkeep, sacrifice it unless you pay its echo cost.)",
      "R: Basalt Gargoyle gets +0/+1 until end of turn."
    ],
    "power": "3",
    "toughness": "2",
    "converted": 3,
    "abilities": 3
  },
  "Big Game Hunter": {
    "name": "Big Game Hunter",
    "color": "Black",
    "cost": "1BB",
    "type": "Creature \u2014 Human Rebel Assassin",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, destroy target creature with power 4 or greater. It can\u2019t be regenerated.",
      ""
    ],
    "textbox": [
      "When Big Game Hunter enters the battlefield, destroy target creature with power 4 or greater. It can\u2019t be regenerated.",
      "Madness B (If you discard this card, discard it into exile. When you do, cast it for its madness cost or put it into your graveyard.)"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 3,
    "abilities": 2
  },
  "Bonded Fetch": {
    "name": "Bonded Fetch",
    "color": "Blue",
    "cost": "2U",
    "type": "Creature \u2014 Homunculus",
    "rarity": "Uncommon",
    "abilitiesText": [
      "T: Draw a card, then discard a card.",
      ""
    ],
    "textbox": [
      "Defender, haste",
      "T: Draw a card, then discard a card."
    ],
    "power": "0",
    "toughness": "2",
    "converted": 3,
    "abilities": 3
  },
  "Brine Elemental": {
    "name": "Brine Elemental",
    "color": "Blue",
    "cost": "4UU",
    "type": "Creature \u2014 Elemental",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card is turned face up, each opponent skips their next untap step.",
      ""
    ],
    "textbox": [
      "Morph 5UU (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)",
      "When Brine Elemental is turned face up, each opponent skips their next untap step."
    ],
    "power": "5",
    "toughness": "4",
    "converted": 6,
    "abilities": 2
  },
  "Calciderm": {
    "name": "Calciderm",
    "color": "White",
    "cost": "2WW",
    "type": "Creature \u2014 Beast",
    "rarity": "Uncommon",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Shroud (This creature can\u2019t be the target of spells or abilities.)",
      "Vanishing 4 (This creature enters the battlefield with four time counters on it. At the beginning of your upkeep, remove a time counter from it. When the last is removed, sacrifice it.)"
    ],
    "power": "5",
    "toughness": "5",
    "converted": 4,
    "abilities": 2
  },
  "Cautery Sliver": {
    "name": "Cautery Sliver",
    "color": "RedWhite",
    "cost": "RW",
    "type": "Creature \u2014 Sliver",
    "rarity": "Uncommon",
    "abilitiesText": [
      "All Slivers have \u201c1, Sacrifice this permanent: This permanent deals 1 damage to any target.\u201d",
      "All Slivers have \u201c1, Sacrifice this permanent: Prevent the next 1 damage that would be dealt to target player, planeswalker, or Sliver creature this turn.\u201d",
      ""
    ],
    "textbox": [
      "All Slivers have \u201c1, Sacrifice this permanent: This permanent deals 1 damage to any target.\u201d",
      "All Slivers have \u201c1, Sacrifice this permanent: Prevent the next 1 damage that would be dealt to target player, planeswalker, or Sliver creature this turn.\u201d"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 2
  },
  "Celestial Crusader": {
    "name": "Celestial Crusader",
    "color": "White",
    "cost": "2WW",
    "type": "Creature \u2014 Spirit",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Other white creatures get +1/+1.",
      ""
    ],
    "textbox": [
      "Flash (You may cast this spell any time you could cast an instant.)",
      "Split second (As long as this spell is on the stack, players can\u2019t cast spells or activate abilities that aren\u2019t mana abilities.)",
      "Flying",
      "Other white creatures get +1/+1."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 4
  },
  "Char-Rumbler": {
    "name": "Char-Rumbler",
    "color": "Red",
    "cost": "2RR",
    "type": "Creature \u2014 Elemental",
    "rarity": "Uncommon",
    "abilitiesText": [
      "R: This card gets +1/+0 until end of turn.",
      ""
    ],
    "textbox": [
      "Double strike",
      "R: Char-Rumbler gets +1/+0 until end of turn."
    ],
    "power": "-1",
    "toughness": "3",
    "converted": 4,
    "abilities": 2
  },
  "Clockwork Hydra": {
    "name": "Clockwork Hydra",
    "color": "Artifact",
    "cost": "5",
    "type": "Artifact Creature \u2014 Hydra",
    "rarity": "Uncommon",
    "abilitiesText": [
      "This card enters the battlefield with four +1/+1 counters on it.",
      "Whenever This card attacks or blocks, remove a +1/+1 counter from it. If you do, This card deals 1 damage to any target.",
      "T: Put a +1/+1 counter on This card.",
      ""
    ],
    "textbox": [
      "Clockwork Hydra enters the battlefield with four +1/+1 counters on it.",
      "Whenever Clockwork Hydra attacks or blocks, remove a +1/+1 counter from it. If you do, Clockwork Hydra deals 1 damage to any target.",
      "T: Put a +1/+1 counter on Clockwork Hydra."
    ],
    "power": "0",
    "toughness": "0",
    "converted": 5,
    "abilities": 3
  },
  "Cloudseeder": {
    "name": "Cloudseeder",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Faerie Spellshaper",
    "rarity": "Uncommon",
    "abilitiesText": [
      "U, T, Discard a card: Create a 1/1 blue Faerie creature token named Cloud Sprite. It has flying and \u201cCloud Sprite can block only creatures with flying.\u201d",
      ""
    ],
    "textbox": [
      "Flying",
      "U, T, Discard a card: Create a 1/1 blue Faerie creature token named Cloud Sprite. It has flying and \u201cCloud Sprite can block only creatures with flying.\u201d"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Cryptic Annelid": {
    "name": "Cryptic Annelid",
    "color": "Blue",
    "cost": "3U",
    "type": "Creature \u2014 Worm Beast",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, scry 1, then scry 2, then scry 3. (To scry X, look at the top X cards of your library, then put any number of them on the bottom of your library and the rest on top in any order.)",
      ""
    ],
    "textbox": [
      "When Cryptic Annelid enters the battlefield, scry 1, then scry 2, then scry 3. (To scry X, look at the top X cards of your library, then put any number of them on the bottom of your library and the rest on top in any order.)"
    ],
    "power": "1",
    "toughness": "4",
    "converted": 4,
    "abilities": 1
  },
  "Darkheart Sliver": {
    "name": "Darkheart Sliver",
    "color": "BlackGreen",
    "cost": "BG",
    "type": "Creature \u2014 Sliver",
    "rarity": "Uncommon",
    "abilitiesText": [
      "All Slivers have \u201cSacrifice this permanent: You gain 3 life.\u201d",
      ""
    ],
    "textbox": [
      "All Slivers have \u201cSacrifice this permanent: You gain 3 life.\u201d"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Dormant Sliver": {
    "name": "Dormant Sliver",
    "color": "GreenBlue",
    "cost": "2GU",
    "type": "Creature \u2014 Sliver",
    "rarity": "Uncommon",
    "abilitiesText": [
      "All Sliver creatures have defender.",
      "All Slivers have \u201cWhen this permanent enters the battlefield, draw a card.\u201d",
      ""
    ],
    "textbox": [
      "All Sliver creatures have defender.",
      "All Slivers have \u201cWhen this permanent enters the battlefield, draw a card.\u201d"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 2
  },
  "Dunerider Outlaw": {
    "name": "Dunerider Outlaw",
    "color": "Black",
    "cost": "BB",
    "type": "Creature \u2014 Human Rebel Rogue",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Protection from green",
      "At the beginning of each end step, if This card dealt damage to an opponent this turn, put a +1/+1 counter on it.",
      ""
    ],
    "textbox": [
      "Protection from green",
      "At the beginning of each end step, if Dunerider Outlaw dealt damage to an opponent this turn, put a +1/+1 counter on it."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Duskrider Peregrine": {
    "name": "Duskrider Peregrine",
    "color": "White",
    "cost": "5W",
    "type": "Creature \u2014 Bird",
    "rarity": "Uncommon",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying, protection from black",
      "Suspend 3\u20141W (Rather than cast this card from your hand, you may pay 1W and exile it with three time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "3",
    "toughness": "3",
    "converted": 6,
    "abilities": 3
  },
  "Faceless Devourer": {
    "name": "Faceless Devourer",
    "color": "Black",
    "cost": "2B",
    "type": "Creature \u2014 Nightmare Horror",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, exile another target creature with shadow.",
      "When This card leaves the battlefield, return the exiled card to the battlefield under its owner\u2019s control.",
      ""
    ],
    "textbox": [
      "Shadow (This creature can block or be blocked by only creatures with shadow.)",
      "When Faceless Devourer enters the battlefield, exile another target creature with shadow.",
      "When Faceless Devourer leaves the battlefield, return the exiled card to the battlefield under its owner\u2019s control."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 3,
    "abilities": 3
  },
  "Firemaw Kavu": {
    "name": "Firemaw Kavu",
    "color": "Red",
    "cost": "5R",
    "type": "Creature \u2014 Kavu",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, it deals 2 damage to target creature.",
      "When This card leaves the battlefield, it deals 4 damage to target creature.",
      ""
    ],
    "textbox": [
      "Echo 5R (At the beginning of your upkeep, if this came under your control since the beginning of your last upkeep, sacrifice it unless you pay its echo cost.)",
      "When Firemaw Kavu enters the battlefield, it deals 2 damage to target creature.",
      "When Firemaw Kavu leaves the battlefield, it deals 4 damage to target creature."
    ],
    "power": "4",
    "toughness": "2",
    "converted": 6,
    "abilities": 3
  },
  "Firewake Sliver": {
    "name": "Firewake Sliver",
    "color": "RedGreen",
    "cost": "1RG",
    "type": "Creature \u2014 Sliver",
    "rarity": "Uncommon",
    "abilitiesText": [
      "All Sliver creatures have haste.",
      "All Slivers have \u201c1, Sacrifice this permanent: Target Sliver creature gets +2/+2 until end of turn.\u201d",
      ""
    ],
    "textbox": [
      "All Sliver creatures have haste.",
      "All Slivers have \u201c1, Sacrifice this permanent: Target Sliver creature gets +2/+2 until end of turn.\u201d"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 3,
    "abilities": 2
  },
  "Fury Sliver": {
    "name": "Fury Sliver",
    "color": "Red",
    "cost": "5R",
    "type": "Creature \u2014 Sliver",
    "rarity": "Uncommon",
    "abilitiesText": [
      "All Sliver creatures have double strike.",
      ""
    ],
    "textbox": [
      "All Sliver creatures have double strike."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 6,
    "abilities": 1
  },
  "Harmonic Sliver": {
    "name": "Harmonic Sliver",
    "color": "GreenWhite",
    "cost": "1GW",
    "type": "Creature \u2014 Sliver",
    "rarity": "Uncommon",
    "abilitiesText": [
      "All Slivers have \u201cWhen this permanent enters the battlefield, destroy target artifact or enchantment.\u201d",
      ""
    ],
    "textbox": [
      "All Slivers have \u201cWhen this permanent enters the battlefield, destroy target artifact or enchantment.\u201d"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 3,
    "abilities": 1
  },
  "Henchfiend of Ukor": {
    "name": "Henchfiend of Ukor",
    "color": "Red",
    "cost": "3R",
    "type": "Creature \u2014 Ogre",
    "rarity": "Uncommon",
    "abilitiesText": [
      "B/R: This card gets +1/+0 until end of turn.",
      ""
    ],
    "textbox": [
      "Haste",
      "Echo 1B (At the beginning of your upkeep, if this came under your control since the beginning of your last upkeep, sacrifice it unless you pay its echo cost.)",
      "B/R: Henchfiend of Ukor gets +1/+0 until end of turn."
    ],
    "power": "3",
    "toughness": "2",
    "converted": 4,
    "abilities": 3
  },
  "Imperiosaur": {
    "name": "Imperiosaur",
    "color": "Green",
    "cost": "2GG",
    "type": "Creature \u2014 Dinosaur",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Spend only mana produced by basic lands to cast this spell.",
      ""
    ],
    "textbox": [
      "Spend only mana produced by basic lands to cast this spell."
    ],
    "power": "5",
    "toughness": "5",
    "converted": 4,
    "abilities": 1
  },
  "Jodah's Avenger": {
    "name": "Jodah's Avenger",
    "color": "Blue",
    "cost": "5U",
    "type": "Creature \u2014 Shapeshifter",
    "rarity": "Uncommon",
    "abilitiesText": [
      "0: Until end of turn, This card gets -1/-1 and gains your choice of double strike, protection from red, vigilance, or shadow. (A creature with shadow can block or be blocked by only creatures with shadow.)",
      ""
    ],
    "textbox": [
      "0: Until end of turn, Jodah\u2019s Avenger gets -1/-1 and gains your choice of double strike, protection from red, vigilance, or shadow. (A creature with shadow can block or be blocked by only creatures with shadow.)"
    ],
    "power": "4",
    "toughness": "4",
    "converted": 6,
    "abilities": 1
  },
  "Knight of the Holy Nimbus": {
    "name": "Knight of the Holy Nimbus",
    "color": "White",
    "cost": "WW",
    "type": "Creature \u2014 Human Rebel Knight",
    "rarity": "Uncommon",
    "abilitiesText": [
      "If This card would be destroyed, regenerate it.",
      "2: This card can\u2019t be regenerated this turn. Only any opponent may activate this ability.",
      ""
    ],
    "textbox": [
      "Flanking (Whenever a creature without flanking blocks this creature, the blocking creature gets -1/-1 until end of turn.)",
      "If Knight of the Holy Nimbus would be destroyed, regenerate it.",
      "2: Knight of the Holy Nimbus can\u2019t be regenerated this turn. Only any opponent may activate this ability."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 3
  },
  "Llanowar Mentor": {
    "name": "Llanowar Mentor",
    "color": "Green",
    "cost": "G",
    "type": "Creature \u2014 Elf Spellshaper",
    "rarity": "Uncommon",
    "abilitiesText": [
      "G, T, Discard a card: Create a 1/1 green Elf Druid creature token named Llanowar Elves. It has \u201cT: Add G.\u201d",
      ""
    ],
    "textbox": [
      "G, T, Discard a card: Create a 1/1 green Elf Druid creature token named Llanowar Elves. It has \u201cT: Add G.\u201d"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 1
  },
  "Lost Auramancers": {
    "name": "Lost Auramancers",
    "color": "White",
    "cost": "2WW",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card dies, if it had no time counters on it, you may search your library for an enchantment card and put it onto the battlefield. If you do, shuffle your library.",
      ""
    ],
    "textbox": [
      "Vanishing 3 (This creature enters the battlefield with three time counters on it. At the beginning of your upkeep, remove a time counter from it. When the last is removed, sacrifice it.)",
      "When Lost Auramancers dies, if it had no time counters on it, you may search your library for an enchantment card and put it onto the battlefield. If you do, shuffle your library."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 4,
    "abilities": 2
  },
  "Might Sliver": {
    "name": "Might Sliver",
    "color": "Green",
    "cost": "4G",
    "type": "Creature \u2014 Sliver",
    "rarity": "Uncommon",
    "abilitiesText": [
      "All Sliver creatures get +2/+2.",
      ""
    ],
    "textbox": [
      "All Sliver creatures get +2/+2."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 5,
    "abilities": 1
  },
  "Mire Boa": {
    "name": "Mire Boa",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Snake",
    "rarity": "Uncommon",
    "abilitiesText": [
      "G: Regenerate This card.",
      ""
    ],
    "textbox": [
      "Swampwalk (This creature can\u2019t be blocked as long as defending player controls a Swamp.)",
      "G: Regenerate Mire Boa."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Muck Drubb": {
    "name": "Muck Drubb",
    "color": "Black",
    "cost": "3BB",
    "type": "Creature \u2014 Beast",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, change the target of target spell that targets only a single creature to This card.",
      ""
    ],
    "textbox": [
      "Flash (You may cast this spell any time you could cast an instant.)",
      "When Muck Drubb enters the battlefield, change the target of target spell that targets only a single creature to Muck Drubb.",
      "Madness 2B (If you discard this card, discard it into exile. When you do, cast it for its madness cost or put it into your graveyard.)"
    ],
    "power": "3",
    "toughness": "3",
    "converted": 5,
    "abilities": 3
  },
  "Mycologist": {
    "name": "Mycologist",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Human Druid",
    "rarity": "Uncommon",
    "abilitiesText": [
      "At the beginning of your upkeep, put a spore counter on This card.",
      "Remove three spore counters from This card: Create a 1/1 green Saproling creature token.",
      "Sacrifice a Saproling: You gain 2 life.",
      ""
    ],
    "textbox": [
      "At the beginning of your upkeep, put a spore counter on Mycologist.",
      "Remove three spore counters from Mycologist: Create a 1/1 green Saproling creature token.",
      "Sacrifice a Saproling: You gain 2 life."
    ],
    "power": "0",
    "toughness": "2",
    "converted": 2,
    "abilities": 3
  },
  "Necrotic Sliver": {
    "name": "Necrotic Sliver",
    "color": "WhiteBlack",
    "cost": "1WB",
    "type": "Creature \u2014 Sliver",
    "rarity": "Uncommon",
    "abilitiesText": [
      "All Slivers have \u201c3, Sacrifice this permanent: Destroy target permanent.\u201d",
      ""
    ],
    "textbox": [
      "All Slivers have \u201c3, Sacrifice this permanent: Destroy target permanent.\u201d"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Nightshade Assassin": {
    "name": "Nightshade Assassin",
    "color": "Black",
    "cost": "2BB",
    "type": "Creature \u2014 Human Assassin",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, you may reveal X black cards in your hand. If you do, target creature gets -X/-X until end of turn.",
      ""
    ],
    "textbox": [
      "First strike",
      "When Nightshade Assassin enters the battlefield, you may reveal X black cards in your hand. If you do, target creature gets -X/-X until end of turn.",
      "Madness 1B (If you discard this card, discard it into exile. When you do, cast it for its madness cost or put it into your graveyard.)"
    ],
    "power": "2",
    "toughness": "1",
    "converted": 4,
    "abilities": 3
  },
  "Outrider en-Kor": {
    "name": "Outrider en-Kor",
    "color": "White",
    "cost": "2W",
    "type": "Creature \u2014 Kor Rebel Knight",
    "rarity": "Uncommon",
    "abilitiesText": [
      "0: The next 1 damage that would be dealt to This card this turn is dealt to target creature you control instead.",
      ""
    ],
    "textbox": [
      "Flanking (Whenever a creature without flanking blocks this creature, the blocking creature gets -1/-1 until end of turn.)",
      "0: The next 1 damage that would be dealt to Outrider en-Kor this turn is dealt to target creature you control instead."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 2
  },
  "Pendelhaven Elder": {
    "name": "Pendelhaven Elder",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Elf Shaman",
    "rarity": "Uncommon",
    "abilitiesText": [
      "T: Each 1/1 creature you control gets +1/+2 until end of turn.",
      ""
    ],
    "textbox": [
      "T: Each 1/1 creature you control gets +1/+2 until end of turn."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Phantom Wurm": {
    "name": "Phantom Wurm",
    "color": "Green",
    "cost": "4GG",
    "type": "Creature \u2014 Wurm Spirit",
    "rarity": "Uncommon",
    "abilitiesText": [
      "This card enters the battlefield with four +1/+1 counters on it.",
      "If damage would be dealt to This card, prevent that damage. Remove a +1/+1 counter from This card.",
      ""
    ],
    "textbox": [
      "Phantom Wurm enters the battlefield with four +1/+1 counters on it.",
      "If damage would be dealt to Phantom Wurm, prevent that damage. Remove a +1/+1 counter from Phantom Wurm."
    ],
    "power": "2",
    "toughness": "0",
    "converted": 6,
    "abilities": 2
  },
  "Poultice Sliver": {
    "name": "Poultice Sliver",
    "color": "White",
    "cost": "2W",
    "type": "Creature \u2014 Sliver",
    "rarity": "Uncommon",
    "abilitiesText": [
      "All Slivers have \u201c2, T: Regenerate target Sliver.\u201d",
      ""
    ],
    "textbox": [
      "All Slivers have \u201c2, T: Regenerate target Sliver.\u201d"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Primal Forcemage": {
    "name": "Primal Forcemage",
    "color": "Green",
    "cost": "2G",
    "type": "Creature \u2014 Elf Shaman",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Whenever another creature enters the battlefield under your control, that creature gets +3/+3 until end of turn.",
      ""
    ],
    "textbox": [
      "Whenever another creature enters the battlefield under your control, that creature gets +3/+3 until end of turn."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Prodigal Pyromancer": {
    "name": "Prodigal Pyromancer",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Uncommon",
    "abilitiesText": [
      "T: This card deals 1 damage to any target.",
      ""
    ],
    "textbox": [
      "T: Prodigal Pyromancer deals 1 damage to any target."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 3,
    "abilities": 1
  },
  "Riftmarked Knight": {
    "name": "Riftmarked Knight",
    "color": "White",
    "cost": "1WW",
    "type": "Creature \u2014 Human Rebel Knight",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Protection from black; flanking (Whenever a creature without flanking blocks this creature, the blocking creature gets -1/-1 until end of turn.)",
      "When the last time counter is removed from This card while it\u2019s exiled, create a 2/2 black Knight creature token with flanking, protection from white, and haste.",
      ""
    ],
    "textbox": [
      "Protection from black; flanking (Whenever a creature without flanking blocks this creature, the blocking creature gets -1/-1 until end of turn.)",
      "Suspend 3\u20141WW (Rather than cast this card from your hand, you may pay 1WW and exile it with three time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)",
      "When the last time counter is removed from Riftmarked Knight while it\u2019s exiled, create a 2/2 black Knight creature token with flanking, protection from white, and haste."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 3
  },
  "Riftwing Cloudskate": {
    "name": "Riftwing Cloudskate",
    "color": "Blue",
    "cost": "3UU",
    "type": "Creature \u2014 Illusion",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, return target permanent to its owner\u2019s hand.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Riftwing Cloudskate enters the battlefield, return target permanent to its owner\u2019s hand.",
      "Suspend 3\u20141U (Rather than cast this card from your hand, you may pay 1U and exile it with three time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 5,
    "abilities": 3
  },
  "Riptide Pilferer": {
    "name": "Riptide Pilferer",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Merfolk Rogue",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Whenever This card deals combat damage to a player, that player discards a card.",
      ""
    ],
    "textbox": [
      "Whenever Riptide Pilferer deals combat damage to a player, that player discards a card.",
      "Morph U (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Saltfield Recluse": {
    "name": "Saltfield Recluse",
    "color": "White",
    "cost": "2W",
    "type": "Creature \u2014 Human Rebel Cleric",
    "rarity": "Uncommon",
    "abilitiesText": [
      "T: Target creature gets -2/-0 until end of turn.",
      ""
    ],
    "textbox": [
      "T: Target creature gets -2/-0 until end of turn."
    ],
    "power": "1",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Scryb Ranger": {
    "name": "Scryb Ranger",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Faerie",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Return a Forest you control to its owner\u2019s hand: Untap target creature. Activate this ability only once each turn.",
      ""
    ],
    "textbox": [
      "Flash (You may cast this spell any time you could cast an instant.)",
      "Flying, protection from blue",
      "Return a Forest you control to its owner\u2019s hand: Untap target creature. Activate this ability only once each turn."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 4
  },
  "Shaper Parasite": {
    "name": "Shaper Parasite",
    "color": "Blue",
    "cost": "1UU",
    "type": "Creature \u2014 Illusion",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card is turned face up, target creature gets +2/-2 or -2/+2 until end of turn.",
      ""
    ],
    "textbox": [
      "Morph 2U (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)",
      "When Shaper Parasite is turned face up, target creature gets +2/-2 or -2/+2 until end of turn."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 3,
    "abilities": 2
  },
  "Shivan Sand-Mage": {
    "name": "Shivan Sand-Mage",
    "color": "Red",
    "cost": "2RR",
    "type": "Creature \u2014 Viashino Shaman",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, choose one \u2014\n\u2022 Remove two time counters from target permanent or suspended card.\n\u2022 Put two time counters on target permanent with a time counter on it or suspended card."
    ],
    "textbox": [
      "When Shivan Sand-Mage enters the battlefield, choose one \u2014",
      "\u2022 Remove two time counters from target permanent or suspended card.",
      "\u2022 Put two time counters on target permanent with a time counter on it or suspended card.",
      "Suspend 4\u2014R (Rather than cast this card from your hand, you may pay R and exile it with four time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "3",
    "toughness": "2",
    "converted": 4,
    "abilities": 2
  },
  "Skittering Monstrosity": {
    "name": "Skittering Monstrosity",
    "color": "Black",
    "cost": "3BB",
    "type": "Creature \u2014 Horror",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When you cast a creature spell, sacrifice This card.",
      ""
    ],
    "textbox": [
      "When you cast a creature spell, sacrifice Skittering Monstrosity."
    ],
    "power": "5",
    "toughness": "5",
    "converted": 5,
    "abilities": 1
  },
  "Sliversmith": {
    "name": "Sliversmith",
    "color": "Artifact",
    "cost": "2",
    "type": "Artifact Creature \u2014 Spellshaper",
    "rarity": "Uncommon",
    "abilitiesText": [
      "1, T, Discard a card: Create a 1/1 colorless Sliver artifact creature token named Metallic Sliver.",
      ""
    ],
    "textbox": [
      "1, T, Discard a card: Create a 1/1 colorless Sliver artifact creature token named Metallic Sliver."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Sporesower Thallid": {
    "name": "Sporesower Thallid",
    "color": "Green",
    "cost": "2GG",
    "type": "Creature \u2014 Fungus",
    "rarity": "Uncommon",
    "abilitiesText": [
      "At the beginning of your upkeep, put a spore counter on each Fungus you control.",
      "Remove three spore counters from This card: Create a 1/1 green Saproling creature token.",
      ""
    ],
    "textbox": [
      "At the beginning of your upkeep, put a spore counter on each Fungus you control.",
      "Remove three spore counters from Sporesower Thallid: Create a 1/1 green Saproling creature token."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 4,
    "abilities": 2
  },
  "Stonecloaker": {
    "name": "Stonecloaker",
    "color": "White",
    "cost": "2W",
    "type": "Creature \u2014 Gargoyle",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, return a creature you control to its owner\u2019s hand.",
      "When This card enters the battlefield, exile target card from a graveyard.",
      ""
    ],
    "textbox": [
      "Flash (You may cast this spell any time you could cast an instant.)",
      "Flying",
      "When Stonecloaker enters the battlefield, return a creature you control to its owner\u2019s hand.",
      "When Stonecloaker enters the battlefield, exile target card from a graveyard."
    ],
    "power": "3",
    "toughness": "2",
    "converted": 3,
    "abilities": 4
  },
  "Stormcloud Djinn": {
    "name": "Stormcloud Djinn",
    "color": "Blue",
    "cost": "4U",
    "type": "Creature \u2014 Djinn",
    "rarity": "Uncommon",
    "abilitiesText": [
      "This card can block only creatures with flying.",
      "RR: This card gets +2/+0 until end of turn and deals 1 damage to you.",
      ""
    ],
    "textbox": [
      "Flying",
      "Stormcloud Djinn can block only creatures with flying.",
      "RR: Stormcloud Djinn gets +2/+0 until end of turn and deals 1 damage to you."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 5,
    "abilities": 3
  },
  "Storm Entity": {
    "name": "Storm Entity",
    "color": "Red",
    "cost": "1R",
    "type": "Creature \u2014 Elemental",
    "rarity": "Uncommon",
    "abilitiesText": [
      "This card enters the battlefield with a +1/+1 counter on it for each other spell cast this turn.",
      ""
    ],
    "textbox": [
      "Haste",
      "Storm Entity enters the battlefield with a +1/+1 counter on it for each other spell cast this turn."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Stormfront Riders": {
    "name": "Stormfront Riders",
    "color": "White",
    "cost": "4W",
    "type": "Creature \u2014 Human Soldier",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, return two creatures you control to their owner\u2019s hand.",
      "Whenever This card or another creature is returned to your hand from the battlefield, create a 1/1 white Soldier creature token.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Stormfront Riders enters the battlefield, return two creatures you control to their owner\u2019s hand.",
      "Whenever Stormfront Riders or another creature is returned to your hand from the battlefield, create a 1/1 white Soldier creature token."
    ],
    "power": "4",
    "toughness": "3",
    "converted": 5,
    "abilities": 3
  },
  "Street Wraith": {
    "name": "Street Wraith",
    "color": "Black",
    "cost": "3BB",
    "type": "Creature \u2014 Wraith",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Cycling\u2014Pay 2 life. (Pay 2 life, Discard this card: Draw a card.)",
      ""
    ],
    "textbox": [
      "Swampwalk (This creature can\u2019t be blocked as long as defending player controls a Swamp.)",
      "Cycling\u2014Pay 2 life. (Pay 2 life, Discard this card: Draw a card.)"
    ],
    "power": "3",
    "toughness": "4",
    "converted": 5,
    "abilities": 2
  },
  "Stronghold Rats": {
    "name": "Stronghold Rats",
    "color": "Black",
    "cost": "2B",
    "type": "Creature \u2014 Rat",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Whenever This card deals combat damage to a player, each player discards a card.",
      ""
    ],
    "textbox": [
      "Shadow (This creature can block or be blocked by only creatures with shadow.)",
      "Whenever Stronghold Rats deals combat damage to a player, each player discards a card."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 3,
    "abilities": 2
  },
  "Sulfur Elemental": {
    "name": "Sulfur Elemental",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Elemental",
    "rarity": "Uncommon",
    "abilitiesText": [
      "White creatures get +1/-1.",
      ""
    ],
    "textbox": [
      "Flash (You may cast this spell any time you could cast an instant.)",
      "Split second (As long as this spell is on the stack, players can\u2019t cast spells or activate abilities that aren\u2019t mana abilities.)",
      "White creatures get +1/-1."
    ],
    "power": "3",
    "toughness": "2",
    "converted": 3,
    "abilities": 3
  },
  "Thick-Skinned Goblin": {
    "name": "Thick-Skinned Goblin",
    "color": "Red",
    "cost": "1R",
    "type": "Creature \u2014 Goblin Shaman",
    "rarity": "Uncommon",
    "abilitiesText": [
      "You may pay 0 rather than pay the echo cost for permanents you control.",
      "R: This card gains protection from red until end of turn.",
      ""
    ],
    "textbox": [
      "You may pay 0 rather than pay the echo cost for permanents you control.",
      "R: Thick-Skinned Goblin gains protection from red until end of turn."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Timebender": {
    "name": "Timebender",
    "color": "Blue",
    "cost": "U",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card is turned face up, choose one \u2014\n\u2022 Remove two time counters from target permanent or suspended card.\n\u2022 Put two time counters on target permanent with a time counter on it or suspended card."
    ],
    "textbox": [
      "Morph U (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)",
      "When Timebender is turned face up, choose one \u2014",
      "\u2022 Remove two time counters from target permanent or suspended card.",
      "\u2022 Put two time counters on target permanent with a time counter on it or suspended card."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 2
  },
  "Utopia Mycon": {
    "name": "Utopia Mycon",
    "color": "Green",
    "cost": "G",
    "type": "Creature \u2014 Fungus",
    "rarity": "Uncommon",
    "abilitiesText": [
      "At the beginning of your upkeep, put a spore counter on This card.",
      "Remove three spore counters from This card: Create a 1/1 green Saproling creature token.",
      "Sacrifice a Saproling: Add one mana of any color.",
      ""
    ],
    "textbox": [
      "At the beginning of your upkeep, put a spore counter on Utopia Mycon.",
      "Remove three spore counters from Utopia Mycon: Create a 1/1 green Saproling creature token.",
      "Sacrifice a Saproling: Add one mana of any color."
    ],
    "power": "0",
    "toughness": "2",
    "converted": 1,
    "abilities": 3
  },
  "Whip-Spine Drake": {
    "name": "Whip-Spine Drake",
    "color": "Blue",
    "cost": "3UU",
    "type": "Creature \u2014 Drake",
    "rarity": "Uncommon",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying",
      "Morph 2W (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)"
    ],
    "power": "3",
    "toughness": "3",
    "converted": 5,
    "abilities": 2
  },
  "Yavimaya Dryad": {
    "name": "Yavimaya Dryad",
    "color": "Green",
    "cost": "1GG",
    "type": "Creature \u2014 Dryad",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, you may search your library for a Forest card and put it onto the battlefield tapped under target player\u2019s control. If you do, shuffle your library.",
      ""
    ],
    "textbox": [
      "Forestwalk (This creature can\u2019t be blocked as long as defending player controls a Forest.)",
      "When Yavimaya Dryad enters the battlefield, you may search your library for a Forest card and put it onto the battlefield tapped under target player\u2019s control. If you do, shuffle your library."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 3,
    "abilities": 2
  },
  "Yixlid Jailer": {
    "name": "Yixlid Jailer",
    "color": "Black",
    "cost": "1B",
    "type": "Creature \u2014 Zombie Wizard",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Cards in graveyards lose all abilities.",
      ""
    ],
    "textbox": [
      "Cards in graveyards lose all abilities."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "keywordsUncommon": {
    "abilitiesText": [
      "Flash",
      "Flying",
      "Echo 2r",
      "Madness b",
      "Defender",
      "Haste",
      "Morph 5uu",
      "Shroud",
      "Vanishing 4",
      "Split second",
      "Double strike",
      "Protection from black",
      "Suspend 3\u20141w",
      "Shadow",
      "Echo 5r",
      "Echo 1b",
      "Flanking",
      "Forestwalk",
      "Vanishing 3",
      "Swampwalk",
      "Madness 2b",
      "First strike",
      "Madness 1b",
      "Suspend 3\u20141ww",
      "Suspend 3\u20141u",
      "Morph u",
      "Protection from blue",
      "Morph 2u",
      "Suspend 4\u2014r",
      "Morph 2w"
    ]
  }
}

tsrRare = {
  "Aeon Chronicler": {
    "name": "Aeon Chronicler",
    "color": "Blue",
    "cost": "3UU",
    "type": "Creature \u2014 Avatar",
    "rarity": "Rare",
    "abilitiesText": [
      "This card\u2019s power and toughness are each equal to the number of cards in your hand.",
      "Suspend X\u2014X3U. X can\u2019t be 0. (Rather than cast this card from your hand, you may pay X3U and exile it with X time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)",
      "Whenever a time counter is removed from This card while it\u2019s exiled, draw a card.",
      ""
    ],
    "textbox": [
      "Aeon Chronicler\u2019s power and toughness are each equal to the number of cards in your hand.",
      "Suspend X\u2014X3U. X can\u2019t be 0. (Rather than cast this card from your hand, you may pay X3U and exile it with X time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)",
      "Whenever a time counter is removed from Aeon Chronicler while it\u2019s exiled, draw a card."
    ],
    "power": "*",
    "toughness": "*",
    "converted": 5,
    "abilities": 3
  },
  "Angel of Salvation": {
    "name": "Angel of Salvation",
    "color": "White",
    "cost": "6WW",
    "type": "Creature \u2014 Angel",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card enters the battlefield, prevent the next 5 damage that would be dealt this turn to any number of targets, divided as you choose.",
      ""
    ],
    "textbox": [
      "Flash; convoke (Your creatures can help cast this spell. Each creature you tap while casting this spell pays for 1 or one mana of that creature\u2019s color.)",
      "Flying",
      "When Angel of Salvation enters the battlefield, prevent the next 5 damage that would be dealt this turn to any number of targets, divided as you choose."
    ],
    "power": "5",
    "toughness": "5",
    "converted": 8,
    "abilities": 4
  },
  "Benalish Commander": {
    "name": "Benalish Commander",
    "color": "White",
    "cost": "3W",
    "type": "Creature \u2014 Human Soldier",
    "rarity": "Rare",
    "abilitiesText": [
      "This card\u2019s power and toughness are each equal to the number of Soldiers you control.",
      "Suspend X\u2014XWW. X can\u2019t be 0. (Rather than cast this card from your hand, you may pay XWW and exile it with X time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)",
      "Whenever a time counter is removed from This card while it\u2019s exiled, create a 1/1 white Soldier creature token.",
      ""
    ],
    "textbox": [
      "Benalish Commander\u2019s power and toughness are each equal to the number of Soldiers you control.",
      "Suspend X\u2014XWW. X can\u2019t be 0. (Rather than cast this card from your hand, you may pay XWW and exile it with X time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)",
      "Whenever a time counter is removed from Benalish Commander while it\u2019s exiled, create a 1/1 white Soldier creature token."
    ],
    "power": "*",
    "toughness": "*",
    "converted": 4,
    "abilities": 3
  },
  "Draining Whelk": {
    "name": "Draining Whelk",
    "color": "Blue",
    "cost": "4UU",
    "type": "Creature \u2014 Illusion",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card enters the battlefield, counter target spell. Put X +1/+1 counters on This card, where X is that spell\u2019s converted mana cost.",
      ""
    ],
    "textbox": [
      "Flash (You may cast this spell any time you could cast an instant.)",
      "Flying",
      "When Draining Whelk enters the battlefield, counter target spell. Put X +1/+1 counters on Draining Whelk, where X is that spell\u2019s converted mana cost."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 6,
    "abilities": 3
  },
  "Dralnu, Lich Lord": {
    "name": "Dralnu, Lich Lord",
    "color": "BlueBlack",
    "cost": "3UB",
    "type": "Legendary Creature \u2014 Zombie Wizard",
    "rarity": "Rare",
    "abilitiesText": [
      "If damage would be dealt to This card, sacrifice that many permanents instead.",
      "T: Target instant or sorcery card in your graveyard gains flashback until end of turn. The flashback cost is equal to its mana cost. (You may cast that card from your graveyard for its flashback cost. Then exile it.)",
      ""
    ],
    "textbox": [
      "If damage would be dealt to Dralnu, Lich Lord, sacrifice that many permanents instead.",
      "T: Target instant or sorcery card in your graveyard gains flashback until end of turn. The flashback cost is equal to its mana cost. (You may cast that card from your graveyard for its flashback cost. Then exile it.)"
    ],
    "power": "3",
    "toughness": "3",
    "converted": 5,
    "abilities": 2
  },
  "Dryad Arbor": {
    "name": "Dry",
    "color": "Gold",
    "cost": "d Arbor",
    "type": "Land Creature \u2014 Forest Dryad",
    "rarity": "Rare",
    "abilitiesText": [
      "(This card isn\u2019t a spell, it\u2019s affected by summoning sickness, and it has \u201cT: Add G.\u201d)",
      ""
    ],
    "textbox": [
      "(Dryad Arbor isn\u2019t a spell, it\u2019s affected by summoning sickness, and it has \u201cT: Add G.\u201d)"
    ],
    "power": "1",
    "toughness": "1",
    "converted": 0,
    "abilities": 1
  },
  "Fungus Sliver": {
    "name": "Fungus Sliver",
    "color": "Green",
    "cost": "3G",
    "type": "Creature \u2014 Fungus Sliver",
    "rarity": "Rare",
    "abilitiesText": [
      "All Sliver creatures have \u201cWhenever this creature is dealt damage, put a +1/+1 counter on it.\u201d (The damage is dealt before the counter is put on.)",
      ""
    ],
    "textbox": [
      "All Sliver creatures have \u201cWhenever this creature is dealt damage, put a +1/+1 counter on it.\u201d (The damage is dealt before the counter is put on.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 1
  },
  "Greater Gargadon": {
    "name": "Greater Gargadon",
    "color": "Red",
    "cost": "9R",
    "type": "Creature \u2014 Beast",
    "rarity": "Rare",
    "abilitiesText": [
      "Sacrifice an artifact, creature, or land: Remove a time counter from This card. Activate this ability only if This card is suspended.",
      ""
    ],
    "textbox": [
      "Suspend 10\u2014R",
      "Sacrifice an artifact, creature, or land: Remove a time counter from Greater Gargadon. Activate this ability only if Greater Gargadon is suspended."
    ],
    "power": "9",
    "toughness": "7",
    "converted": 10,
    "abilities": 2
  },
  "Heartwood Storyteller": {
    "name": "Heartwood Storyteller",
    "color": "Green",
    "cost": "1GG",
    "type": "Creature \u2014 Treefolk",
    "rarity": "Rare",
    "abilitiesText": [
      "Whenever a player casts a noncreature spell, each of that player\u2019s opponents may draw a card.",
      ""
    ],
    "textbox": [
      "Whenever a player casts a noncreature spell, each of that player\u2019s opponents may draw a card."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 3,
    "abilities": 1
  },
  "Ith, High Arcanist": {
    "name": "Ith, High Arcanist",
    "color": "WhiteBlue",
    "cost": "5WU",
    "type": "Legendary Creature \u2014 Human Wizard",
    "rarity": "Rare",
    "abilitiesText": [
      "T: Untap target attacking creature. Prevent all combat damage that would be dealt to and dealt by that creature this turn.",
      ""
    ],
    "textbox": [
      "Vigilance",
      "T: Untap target attacking creature. Prevent all combat damage that would be dealt to and dealt by that creature this turn.",
      "Suspend 4\u2014WU (Rather than cast this card from your hand, you may pay WU and exile it with four time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "3",
    "toughness": "5",
    "converted": 7,
    "abilities": 3
  },
  "Jaya Ballard, Task Mage": {
    "name": "Jaya Ballard, Task Mage",
    "color": "Red",
    "cost": "1RR",
    "type": "Legendary Creature \u2014 Human Spellshaper",
    "rarity": "Rare",
    "abilitiesText": [
      "R, T, Discard a card: Destroy target blue permanent.",
      "1R, T, Discard a card: This card deals 3 damage to any target. A creature dealt damage this way can\u2019t be regenerated this turn.",
      "5RR, T, Discard a card: This card deals 6 damage to each creature and each player.",
      ""
    ],
    "textbox": [
      "R, T, Discard a card: Destroy target blue permanent.",
      "1R, T, Discard a card: Jaya Ballard, Task Mage deals 3 damage to any target. A creature dealt damage this way can\u2019t be regenerated this turn.",
      "5RR, T, Discard a card: Jaya Ballard deals 6 damage to each creature and each player."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 3
  },
  "Jhoira of the Ghitu": {
    "name": "Jhoira of the Ghitu",
    "color": "BlueRed",
    "cost": "1UR",
    "type": "Legendary Creature \u2014 Human Wizard",
    "rarity": "Rare",
    "abilitiesText": [
      "2, Exile a nonland card from your hand: Put four time counters on the exiled card. If it doesn\u2019t have suspend, it gains suspend. (At the beginning of your upkeep, remove a time counter from that card. When the last is removed, cast it without paying its mana cost. If it\u2019s a creature, it has haste.)",
      ""
    ],
    "textbox": [
      "2, Exile a nonland card from your hand: Put four time counters on the exiled card. If it doesn\u2019t have suspend, it gains suspend. (At the beginning of your upkeep, remove a time counter from that card. When the last is removed, cast it without paying its mana cost. If it\u2019s a creature, it has haste.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Kaervek the Merciless": {
    "name": "Kaervek the Merciless",
    "color": "BlackRed",
    "cost": "5BR",
    "type": "Legendary Creature \u2014 Human Shaman",
    "rarity": "Rare",
    "abilitiesText": [
      "Whenever an opponent casts a spell, This card deals damage to any target equal to that spell\u2019s converted mana cost.",
      ""
    ],
    "textbox": [
      "Whenever an opponent casts a spell, Kaervek the Merciless deals damage to any target equal to that spell\u2019s converted mana cost."
    ],
    "power": "5",
    "toughness": "4",
    "converted": 7,
    "abilities": 1
  },
  "Magus of the Future": {
    "name": "Magus of the Future",
    "color": "Blue",
    "cost": "2UUU",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Rare",
    "abilitiesText": [
      "Play with the top card of your library revealed.",
      "You may play lands and cast spells from the top of your library.",
      ""
    ],
    "textbox": [
      "Play with the top card of your library revealed.",
      "You may play lands and cast spells from the top of your library."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 5,
    "abilities": 2
  },
  "Magus of the Moon": {
    "name": "Magus of the Moon",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Rare",
    "abilitiesText": [
      "Nonbasic lands are Mountains.",
      ""
    ],
    "textbox": [
      "Nonbasic lands are Mountains."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Mangara of Corondor": {
    "name": "Mangara of Corondor",
    "color": "White",
    "cost": "1WW",
    "type": "Legendary Creature \u2014 Human Wizard",
    "rarity": "Rare",
    "abilitiesText": [
      "T: Exile This card and target permanent.",
      ""
    ],
    "textbox": [
      "T: Exile Mangara of Corondor and target permanent."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 3,
    "abilities": 1
  },
  "Mirri the Cursed": {
    "name": "Mirri the Cursed",
    "color": "Black",
    "cost": "2BB",
    "type": "Legendary Creature \u2014 Vampire Cat",
    "rarity": "Rare",
    "abilitiesText": [
      "Whenever This card deals combat damage to a creature, put a +1/+1 counter on This card.",
      ""
    ],
    "textbox": [
      "Flying, first strike, haste",
      "Whenever Mirri the Cursed deals combat damage to a creature, put a +1/+1 counter on Mirri the Cursed."
    ],
    "power": "3",
    "toughness": "2",
    "converted": 4,
    "abilities": 4
  },
  "Nether Traitor": {
    "name": "Nether Traitor",
    "color": "Black",
    "cost": "BB",
    "type": "Creature \u2014 Spirit",
    "rarity": "Rare",
    "abilitiesText": [
      "Whenever another creature is put into your graveyard from the battlefield, you may pay B. If you do, return This card from your graveyard to the battlefield.",
      ""
    ],
    "textbox": [
      "Haste",
      "Shadow (This creature can block or be blocked by only creatures with shadow.)",
      "Whenever another creature is put into your graveyard from the battlefield, you may pay B. If you do, return Nether Traitor from your graveyard to the battlefield."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 3
  },
  "Pulmonic Sliver": {
    "name": "Pulmonic Sliver",
    "color": "White",
    "cost": "3WW",
    "type": "Creature \u2014 Sliver",
    "rarity": "Rare",
    "abilitiesText": [
      "All Sliver creatures have flying.",
      "All Slivers have \u201cIf this permanent would be put into a graveyard, you may put it on top of its owner\u2019s library instead.\u201d",
      ""
    ],
    "textbox": [
      "All Sliver creatures have flying.",
      "All Slivers have \u201cIf this permanent would be put into a graveyard, you may put it on top of its owner\u2019s library instead.\u201d"
    ],
    "power": "3",
    "toughness": "3",
    "converted": 5,
    "abilities": 2
  },
  "Radha, Heir to Keld": {
    "name": "Radha, Heir to Keld",
    "color": "RedGreen",
    "cost": "RG",
    "type": "Legendary Creature \u2014 Elf Warrior",
    "rarity": "Rare",
    "abilitiesText": [
      "Whenever This card attacks, you may add RR.",
      "T: Add G.",
      ""
    ],
    "textbox": [
      "Whenever Radha, Heir to Keld attacks, you may add RR.",
      "T: Add G."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 2
  },
  "Saffi Eriksdotter": {
    "name": "Saffi Eriksdotter",
    "color": "GreenWhite",
    "cost": "GW",
    "type": "Legendary Creature \u2014 Human Scout",
    "rarity": "Rare",
    "abilitiesText": [
      "Sacrifice This card: When target creature is put into your graveyard from the battlefield this turn, return that card to the battlefield.",
      ""
    ],
    "textbox": [
      "Sacrifice Saffi Eriksdotter: When target creature is put into your graveyard from the battlefield this turn, return that card to the battlefield."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Sedge Sliver": {
    "name": "Sedge Sliver",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Sliver",
    "rarity": "Rare",
    "abilitiesText": [
      "All Sliver creatures have \u201cThis creature gets +1/+1 as long as you control a Swamp.\u201d",
      "All Slivers have \u201cB: Regenerate this permanent.\u201d",
      ""
    ],
    "textbox": [
      "All Sliver creatures have \u201cThis creature gets +1/+1 as long as you control a Swamp.\u201d",
      "All Slivers have \u201cB: Regenerate this permanent.\u201d"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 2
  },
  "Sengir Nosferatu": {
    "name": "Sengir Nosferatu",
    "color": "Black",
    "cost": "3BB",
    "type": "Creature \u2014 Vampire",
    "rarity": "Rare",
    "abilitiesText": [
      "1B, Exile This card: Create a 1/2 black Bat creature token with flying. It has \u201c1B, Sacrifice this creature: Return an exiled card named This card to the battlefield under its owner\u2019s control.\u201d",
      ""
    ],
    "textbox": [
      "Flying",
      "1B, Exile Sengir Nosferatu: Create a 1/2 black Bat creature token with flying. It has \u201c1B, Sacrifice this creature: Return an exiled card named Sengir Nosferatu to the battlefield under its owner\u2019s control.\u201d"
    ],
    "power": "4",
    "toughness": "4",
    "converted": 5,
    "abilities": 2
  },
  "Serra Avenger": {
    "name": "Serra Avenger",
    "color": "White",
    "cost": "WW",
    "type": "Creature \u2014 Angel",
    "rarity": "Rare",
    "abilitiesText": [
      "You can\u2019t cast this spell during your first, second, or third turns of the game.",
      ""
    ],
    "textbox": [
      "You can\u2019t cast this spell during your first, second, or third turns of the game.",
      "Flying",
      "Vigilance (Attacking doesn\u2019t cause this creature to tap.)"
    ],
    "power": "3",
    "toughness": "3",
    "converted": 2,
    "abilities": 3
  },
  "Stuffy Doll": {
    "name": "Stuffy Doll",
    "color": "Artifact",
    "cost": "5",
    "type": "Artifact Creature \u2014 Construct",
    "rarity": "Rare",
    "abilitiesText": [
      "As This card enters the battlefield, choose a player.",
      "Whenever This card is dealt damage, it deals that much damage to the chosen player.",
      "T: This card deals 1 damage to itself.",
      ""
    ],
    "textbox": [
      "Indestructible",
      "As Stuffy Doll enters the battlefield, choose a player.",
      "Whenever Stuffy Doll is dealt damage, it deals that much damage to the chosen player.",
      "T: Stuffy Doll deals 1 damage to itself."
    ],
    "power": "0",
    "toughness": "1",
    "converted": 5,
    "abilities": 4
  },
  "Thelonite Hermit": {
    "name": "Thelonite Hermit",
    "color": "Green",
    "cost": "3G",
    "type": "Creature \u2014 Elf Shaman",
    "rarity": "Rare",
    "abilitiesText": [
      "All Saprolings get +1/+1.",
      "When This card is turned face up, create four 1/1 green Saproling creature tokens.",
      ""
    ],
    "textbox": [
      "All Saprolings get +1/+1.",
      "Morph 3GG (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)",
      "When Thelonite Hermit is turned face up, create four 1/1 green Saproling creature tokens."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 4,
    "abilities": 3
  },
  "Thelon of Havenwood": {
    "name": "Thelon of Havenwood",
    "color": "Green",
    "cost": "GG",
    "type": "Legendary Creature \u2014 Elf Druid",
    "rarity": "Rare",
    "abilitiesText": [
      "Each Fungus creature gets +1/+1 for each spore counter on it.",
      "BG, Exile a Fungus card from a graveyard: Put a spore counter on each Fungus on the battlefield.",
      ""
    ],
    "textbox": [
      "Each Fungus creature gets +1/+1 for each spore counter on it.",
      "BG, Exile a Fungus card from a graveyard: Put a spore counter on each Fungus on the battlefield."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 2
  },
  "Tombstalker": {
    "name": "Tombstalker",
    "color": "Black",
    "cost": "6BB",
    "type": "Creature \u2014 Demon",
    "rarity": "Rare",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying",
      "Delve (Each card you exile from your graveyard while casting this spell pays for 1.)"
    ],
    "power": "5",
    "toughness": "5",
    "converted": 8,
    "abilities": 2
  },
  "Venser, Shaper Savant": {
    "name": "Venser, Shaper Savant",
    "color": "Blue",
    "cost": "2UU",
    "type": "Legendary Creature \u2014 Human Wizard",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card enters the battlefield, return target spell or permanent to its owner\u2019s hand.",
      ""
    ],
    "textbox": [
      "Flash",
      "When Venser, Shaper Savant enters the battlefield, return target spell or permanent to its owner\u2019s hand."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 2
  },
  "Vesuvan Shapeshifter": {
    "name": "Vesuvan Shapeshifter",
    "color": "Blue",
    "cost": "3UU",
    "type": "Creature \u2014 Shapeshifter",
    "rarity": "Rare",
    "abilitiesText": [
      "As This card enters the battlefield or is turned face up, you may choose another creature on the battlefield. If you do, until This card is turned face down, it becomes a copy of that creature, except it has \u201cAt the beginning of your upkeep, you may turn this creature face down.\u201d",
      ""
    ],
    "textbox": [
      "As Vesuvan Shapeshifter enters the battlefield or is turned face up, you may choose another creature on the battlefield. If you do, until Vesuvan Shapeshifter is turned face down, it becomes a copy of that creature, except it has \u201cAt the beginning of your upkeep, you may turn this creature face down.\u201d",
      "Morph 1U (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)"
    ],
    "power": "0",
    "toughness": "0",
    "converted": 5,
    "abilities": 2
  },
  "keywordsRare": {
    "abilitiesText": [
      "Flash",
      "Convoke",
      "Flying",
      "Flash",
      "Suspend 10\u2014r",
      "Vigilance",
      "Suspend 4\u2014wu",
      "First strike",
      "Haste",
      "Shadow",
      "Indestructible",
      "Morph 3gg",
      "Delve",
      "Morph 1u"
    ]
  }
}

tsrMythic = {
  "Akroma, Angel of Fury": {
    "name": "Akroma, Angel of Fury",
    "color": "Red",
    "cost": "5RRR",
    "type": "Legendary Creature \u2014 Angel",
    "rarity": "Mythic",
    "abilitiesText": [
      "This spell can\u2019t be countered.",
      "R: This card gets +1/+0 until end of turn.",
      ""
    ],
    "textbox": [
      "This spell can\u2019t be countered.",
      "Flying, trample, protection from white and from blue",
      "R: Akroma, Angel of Fury gets +1/+0 until end of turn.",
      "Morph 3RRR (You may cast this card face down as a 2/2 creature for 3. Turn it face up any time for its morph cost.)"
    ],
    "power": "6",
    "toughness": "6",
    "converted": 8,
    "abilities": 6
  },
  "Crovax, Ascendant Hero": {
    "name": "Crovax, Ascendant Hero",
    "color": "White",
    "cost": "4WW",
    "type": "Legendary Creature \u2014 Human Noble",
    "rarity": "Mythic",
    "abilitiesText": [
      "Other white creatures get +1/+1.",
      "Nonwhite creatures get -1/-1.",
      "Pay 2 life: Return This card to its owner\u2019s hand.",
      ""
    ],
    "textbox": [
      "Other white creatures get +1/+1.",
      "Nonwhite creatures get -1/-1.",
      "Pay 2 life: Return Crovax, Ascendant Hero to its owner\u2019s hand."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 6,
    "abilities": 3
  },
  "Sliver Legion": {
    "name": "Sliver Legion",
    "color": "Gold",
    "cost": "WUBRG",
    "type": "Legendary Creature \u2014 Sliver",
    "rarity": "Mythic",
    "abilitiesText": [
      "All Sliver creatures get +1/+1 for each other Sliver on the battlefield.",
      ""
    ],
    "textbox": [
      "All Sliver creatures get +1/+1 for each other Sliver on the battlefield."
    ],
    "power": "7",
    "toughness": "7",
    "converted": 5,
    "abilities": 1
  },
  "Tarmogoyf": {
    "name": "Tarmogoyf",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Lhurgoyf",
    "rarity": "Mythic",
    "abilitiesText": [
      "This card\u2019s power is equal to the number of card types among cards in all graveyards and its toughness is equal to that number plus 1.",
      ""
    ],
    "textbox": [
      "Tarmogoyf\u2019s power is equal to the number of card types among cards in all graveyards and its toughness is equal to that number plus 1."
    ],
    "power": "*",
    "toughness": "1+*",
    "converted": 2,
    "abilities": 1
  },
  "Teferi, Mage of Zhalfir": {
    "name": "Teferi, Mage of Zhalfir",
    "color": "Blue",
    "cost": "2UUU",
    "type": "Legendary Creature \u2014 Human Wizard",
    "rarity": "Mythic",
    "abilitiesText": [
      "Creature cards you own that aren\u2019t on the battlefield have flash.",
      "Each opponent can cast spells only any time they could cast a sorcery.",
      ""
    ],
    "textbox": [
      "Flash (You may cast this spell any time you could cast an instant.)",
      "Creature cards you own that aren\u2019t on the battlefield have flash.",
      "Each opponent can cast spells only any time they could cast a sorcery."
    ],
    "power": "3",
    "toughness": "4",
    "converted": 5,
    "abilities": 3
  },
  "keywordsMythic": {
    "abilitiesText": [
      "Flying",
      "Trample",
      "Protection from white and from blue",
      "Morph 3rrr",
      "Flash"
    ]
  }
}