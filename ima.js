imaCommon = {
  "Ainok Bond-Kin": {
    "name": "Ainok Bond-Kin",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Dog Soldier",
    "rarity": "Common",
    "abilitiesText": [
      "Outlast 1W (1W, T: Put a +1/+1 counter on this creature. Outlast only as a sorcery.)",
      "Each creature you control with a +1/+1 counter on it has first strike.",
      ""
    ],
    "textbox": [
      "Outlast 1W (1W, T: Put a +1/+1 counter on this creature. Outlast only as a sorcery.)",
      "Each creature you control with a +1/+1 counter on it has first strike."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Angel of Mercy": {
    "name": "Angel of Mercy",
    "color": "White",
    "cost": "4W",
    "type": "Creature \u2014 Angel",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, you gain 3 life.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Angel of Mercy enters the battlefield, you gain 3 life."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 5,
    "abilities": 2
  },
  "Bala Ged Scorpion": {
    "name": "Bala Ged Scorpion",
    "color": "Black",
    "cost": "3B",
    "type": "Creature \u2014 Scorpion",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, you may destroy target creature with power 1 or less.",
      ""
    ],
    "textbox": [
      "When Bala Ged Scorpion enters the battlefield, you may destroy target creature with power 1 or less."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 4,
    "abilities": 1
  },
  "Balustrade Spy": {
    "name": "Balustrade Spy",
    "color": "Black",
    "cost": "3B",
    "type": "Creature \u2014 Vampire Rogue",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, target player reveals cards from the top of their library until they reveal a land card, then puts those cards into their graveyard.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Balustrade Spy enters the battlefield, target player reveals cards from the top of their library until they reveal a land card, then puts those cards into their graveyard."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 4,
    "abilities": 2
  },
  "Battle-Rattle Shaman": {
    "name": "Battle-Rattle Shaman",
    "color": "Red",
    "cost": "3R",
    "type": "Creature \u2014 Goblin Shaman",
    "rarity": "Common",
    "abilitiesText": [
      "At the beginning of combat on your turn, you may have target creature get +2/+0 until end of turn.",
      ""
    ],
    "textbox": [
      "At the beginning of combat on your turn, you may have target creature get +2/+0 until end of turn."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 1
  },
  "Benevolent Ancestor": {
    "name": "Benevolent Ancestor",
    "color": "White",
    "cost": "2W",
    "type": "Creature \u2014 Spirit",
    "rarity": "Common",
    "abilitiesText": [
      "T: Prevent the next 1 damage that would be dealt to any target this turn.",
      ""
    ],
    "textbox": [
      "Defender (This creature can\u2019t attack.)",
      "T: Prevent the next 1 damage that would be dealt to any target this turn."
    ],
    "power": "0",
    "toughness": "4",
    "converted": 3,
    "abilities": 2
  },
  "Blinding Mage": {
    "name": "Blinding Mage",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Common",
    "abilitiesText": [
      "W, T: Tap target creature.",
      ""
    ],
    "textbox": [
      "W, T: Tap target creature."
    ],
    "power": "1",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Borderland Marauder": {
    "name": "Borderland Marauder",
    "color": "Red",
    "cost": "1R",
    "type": "Creature \u2014 Human Warrior",
    "rarity": "Common",
    "abilitiesText": [
      "Whenever This card attacks, it gets +2/+0 until end of turn.",
      ""
    ],
    "textbox": [
      "Whenever Borderland Marauder attacks, it gets +2/+0 until end of turn."
    ],
    "power": "1",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Child of Night": {
    "name": "Child of Night",
    "color": "Black",
    "cost": "1B",
    "type": "Creature \u2014 Vampire",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Lifelink"
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Crowned Ceratok": {
    "name": "Crowned Ceratok",
    "color": "Green",
    "cost": "3G",
    "type": "Creature \u2014 Rhino",
    "rarity": "Common",
    "abilitiesText": [
      "Each creature you control with a +1/+1 counter on it has trample.",
      ""
    ],
    "textbox": [
      "Trample",
      "Each creature you control with a +1/+1 counter on it has trample."
    ],
    "power": "4",
    "toughness": "3",
    "converted": 4,
    "abilities": 2
  },
  "Dead Reveler": {
    "name": "Dead Reveler",
    "color": "Black",
    "cost": "2B",
    "type": "Creature \u2014 Zombie",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Unleash (You may have this creature enter the battlefield with a +1/+1 counter on it. It can\u2019t block as long as it has a +1/+1 counter on it.)"
    ],
    "power": "2",
    "toughness": "3",
    "converted": 3,
    "abilities": 1
  },
  "Doomed Traveler": {
    "name": "Doomed Traveler",
    "color": "White",
    "cost": "W",
    "type": "Creature \u2014 Human Soldier",
    "rarity": "Common",
    "abilitiesText": [
      "When This card dies, create a 1/1 white Spirit creature token with flying.",
      ""
    ],
    "textbox": [
      "When Doomed Traveler dies, create a 1/1 white Spirit creature token with flying."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 1
  },
  "Doorkeeper": {
    "name": "Doorkeeper",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Homunculus",
    "rarity": "Common",
    "abilitiesText": [
      "2U, T: Target player mills X cards, where X is the number of creatures with defender you control.",
      ""
    ],
    "textbox": [
      "Defender",
      "2U, T: Target player mills X cards, where X is the number of creatures with defender you control."
    ],
    "power": "0",
    "toughness": "4",
    "converted": 2,
    "abilities": 2
  },
  "Dragon Bell Monk": {
    "name": "Dragon Bell Monk",
    "color": "White",
    "cost": "2W",
    "type": "Creature \u2014 Human Monk",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Vigilance",
      "Prowess (Whenever you cast a noncreature spell, this creature gets +1/+1 until end of turn.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 2
  },
  "Dragon Egg": {
    "name": "Dragon Egg",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Dragon Egg",
    "rarity": "Common",
    "abilitiesText": [
      "When This card dies, create a 2/2 red Dragon creature token with flying and \u201cR: This creature gets +1/+0 until end of turn.\u201d",
      ""
    ],
    "textbox": [
      "Defender",
      "When Dragon Egg dies, create a 2/2 red Dragon creature token with flying and \u201cR: This creature gets +1/+0 until end of turn.\u201d"
    ],
    "power": "0",
    "toughness": "2",
    "converted": 3,
    "abilities": 2
  },
  "Dragonlord's Servant": {
    "name": "Dragonlord's Servant",
    "color": "Red",
    "cost": "1R",
    "type": "Creature \u2014 Goblin Shaman",
    "rarity": "Common",
    "abilitiesText": [
      "Dragon spells you cast cost 1 less to cast.",
      ""
    ],
    "textbox": [
      "Dragon spells you cast cost 1 less to cast."
    ],
    "power": "1",
    "toughness": "3",
    "converted": 2,
    "abilities": 1
  },
  "Durkwood Baloth": {
    "name": "Durkwood Baloth",
    "color": "Green",
    "cost": "4GG",
    "type": "Creature \u2014 Beast",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Suspend 5\u2014G (Rather than cast this card from your hand, you may pay G and exile it with five time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "5",
    "toughness": "5",
    "converted": 6,
    "abilities": 1
  },
  "Duskdale Wurm": {
    "name": "Duskdale Wurm",
    "color": "Green",
    "cost": "5GG",
    "type": "Creature \u2014 Wurm",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Trample (This creature can deal excess combat damage to the player or planeswalker it\u2019s attacking.)"
    ],
    "power": "7",
    "toughness": "7",
    "converted": 7,
    "abilities": 1
  },
  "Earth Elemental": {
    "name": "Earth Elemental",
    "color": "Red",
    "cost": "3RR",
    "type": "Creature \u2014 Elemental",
    "rarity": "Common",
    "abilitiesText": [
      "",
      ""
    ],
    "textbox": [
      ""
    ],
    "power": "4",
    "toughness": "5",
    "converted": 5,
    "abilities": 0
  },
  "Elusive Spellfist": {
    "name": "Elusive Spellfist",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Human Monk",
    "rarity": "Common",
    "abilitiesText": [
      "Whenever you cast a noncreature spell, This card gets +1/+0 until end of turn and can\u2019t be blocked this turn.",
      ""
    ],
    "textbox": [
      "Whenever you cast a noncreature spell, Elusive Spellfist gets +1/+0 until end of turn and can\u2019t be blocked this turn."
    ],
    "power": "1",
    "toughness": "3",
    "converted": 2,
    "abilities": 1
  },
  "Festering Newt": {
    "name": "Festering Newt",
    "color": "Black",
    "cost": "B",
    "type": "Creature \u2014 Salamander",
    "rarity": "Common",
    "abilitiesText": [
      "When This card dies, target creature an opponent controls gets -1/-1 until end of turn. That creature gets -4/-4 instead if you control a creature named Bogbrew Witch.",
      ""
    ],
    "textbox": [
      "When Festering Newt dies, target creature an opponent controls gets -1/-1 until end of turn. That creature gets -4/-4 instead if you control a creature named Bogbrew Witch."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 1
  },
  "Frost Lynx": {
    "name": "Frost Lynx",
    "color": "Blue",
    "cost": "2U",
    "type": "Creature \u2014 Elemental Cat",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, tap target creature an opponent controls. That creature doesn\u2019t untap during its controller\u2019s next untap step.",
      ""
    ],
    "textbox": [
      "When Frost Lynx enters the battlefield, tap target creature an opponent controls. That creature doesn\u2019t untap during its controller\u2019s next untap step."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Furnace Whelp": {
    "name": "Furnace Whelp",
    "color": "Red",
    "cost": "2RR",
    "type": "Creature \u2014 Dragon",
    "rarity": "Common",
    "abilitiesText": [
      "R: This card gets +1/+0 until end of turn.",
      ""
    ],
    "textbox": [
      "Flying (This creature can\u2019t be blocked except by creatures with flying or reach.)",
      "R: Furnace Whelp gets +1/+0 until end of turn."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 2
  },
  "Greater Basilisk": {
    "name": "Greater Basilisk",
    "color": "Green",
    "cost": "3GG",
    "type": "Creature \u2014 Basilisk",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Deathtouch (Any amount of damage this deals to a creature is enough to destroy it.)"
    ],
    "power": "3",
    "toughness": "5",
    "converted": 5,
    "abilities": 1
  },
  "Infantry Veteran": {
    "name": "Infantry Veteran",
    "color": "White",
    "cost": "W",
    "type": "Creature \u2014 Human Soldier",
    "rarity": "Common",
    "abilitiesText": [
      "T: Target attacking creature gets +1/+1 until end of turn.",
      ""
    ],
    "textbox": [
      "T: Target attacking creature gets +1/+1 until end of turn."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 1
  },
  "Ivy Elemental": {
    "name": "Ivy Elemental",
    "color": "Green",
    "cost": "XG",
    "type": "Creature \u2014 Elemental",
    "rarity": "Common",
    "abilitiesText": [
      "This card enters the battlefield with X +1/+1 counters on it.",
      ""
    ],
    "textbox": [
      "Ivy Elemental enters the battlefield with X +1/+1 counters on it."
    ],
    "power": "0",
    "toughness": "0",
    "converted": 2,
    "abilities": 1
  },
  "Jace's Phantasm": {
    "name": "Jace's Phantasm",
    "color": "Blue",
    "cost": "U",
    "type": "Creature \u2014 Illusion",
    "rarity": "Common",
    "abilitiesText": [
      "This card gets +4/+4 as long as an opponent has ten or more cards in their graveyard.",
      ""
    ],
    "textbox": [
      "Flying",
      "Jace\u2019s Phantasm gets +4/+4 as long as an opponent has ten or more cards in their graveyard."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 2
  },
  "Jaddi Offshoot": {
    "name": "Jaddi Offshoot",
    "color": "Green",
    "cost": "G",
    "type": "Creature \u2014 Plant",
    "rarity": "Common",
    "abilitiesText": [
      "Landfall \u2014 Whenever a land enters the battlefield under your control, you gain 1 life.",
      ""
    ],
    "textbox": [
      "Defender",
      "Landfall \u2014 Whenever a land enters the battlefield under your control, you gain 1 life."
    ],
    "power": "0",
    "toughness": "3",
    "converted": 1,
    "abilities": 2
  },
  "Jhessian Thief": {
    "name": "Jhessian Thief",
    "color": "Blue",
    "cost": "2U",
    "type": "Creature \u2014 Human Rogue",
    "rarity": "Common",
    "abilitiesText": [
      "Whenever This card deals combat damage to a player, draw a card.",
      ""
    ],
    "textbox": [
      "Prowess (Whenever you cast a noncreature spell, this creature gets +1/+1 until end of turn.)",
      "Whenever Jhessian Thief deals combat damage to a player, draw a card."
    ],
    "power": "1",
    "toughness": "3",
    "converted": 3,
    "abilities": 2
  },
  "Keldon Halberdier": {
    "name": "Keldon Halberdier",
    "color": "Red",
    "cost": "4R",
    "type": "Creature \u2014 Human Warrior",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "First strike",
      "Suspend 4\u2014R (Rather than cast this card from your hand, you may pay R and exile it with four time counters on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "4",
    "toughness": "1",
    "converted": 5,
    "abilities": 2
  },
  "Kiln Fiend": {
    "name": "Kiln Fiend",
    "color": "Red",
    "cost": "1R",
    "type": "Creature \u2014 Elemental Beast",
    "rarity": "Common",
    "abilitiesText": [
      "Whenever you cast an instant or sorcery spell, This card gets +3/+0 until end of turn.",
      ""
    ],
    "textbox": [
      "Whenever you cast an instant or sorcery spell, Kiln Fiend gets +3/+0 until end of turn."
    ],
    "power": "1",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Manakin": {
    "name": "Manakin",
    "color": "Artifact",
    "cost": "2",
    "type": "Artifact Creature \u2014 Construct",
    "rarity": "Common",
    "abilitiesText": [
      "T: Add C.",
      ""
    ],
    "textbox": [
      "T: Add C."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Mnemonic Wall": {
    "name": "Mnemonic Wall",
    "color": "Blue",
    "cost": "4U",
    "type": "Creature \u2014 Wall",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, you may return target instant or sorcery card from your graveyard to your hand.",
      ""
    ],
    "textbox": [
      "Defender",
      "When Mnemonic Wall enters the battlefield, you may return target instant or sorcery card from your graveyard to your hand."
    ],
    "power": "0",
    "toughness": "4",
    "converted": 5,
    "abilities": 2
  },
  "Nantuko Shaman": {
    "name": "Nantuko Shaman",
    "color": "Green",
    "cost": "2G",
    "type": "Creature \u2014 Insect Shaman",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, if you control no tapped lands, draw a card.",
      ""
    ],
    "textbox": [
      "When Nantuko Shaman enters the battlefield, if you control no tapped lands, draw a card.",
      "Suspend 1\u20142GG (Rather than cast this card from your hand, you may pay 2GG and exile it with a time counter on it. At the beginning of your upkeep, remove a time counter. When the last is removed, cast it without paying its mana cost. It has haste.)"
    ],
    "power": "3",
    "toughness": "2",
    "converted": 3,
    "abilities": 2
  },
  "Netcaster Spider": {
    "name": "Netcaster Spider",
    "color": "Green",
    "cost": "2G",
    "type": "Creature \u2014 Spider",
    "rarity": "Common",
    "abilitiesText": [
      "Whenever This card blocks a creature with flying, This card gets +2/+0 until end of turn.",
      ""
    ],
    "textbox": [
      "Reach (This creature can block creatures with flying.)",
      "Whenever Netcaster Spider blocks a creature with flying, Netcaster Spider gets +2/+0 until end of turn."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 3,
    "abilities": 2
  },
  "Phantom Monster": {
    "name": "Phantom Monster",
    "color": "Blue",
    "cost": "3U",
    "type": "Creature \u2014 Illusion",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying"
    ],
    "power": "3",
    "toughness": "3",
    "converted": 4,
    "abilities": 1
  },
  "Phantom Tiger": {
    "name": "Phantom Tiger",
    "color": "Green",
    "cost": "2G",
    "type": "Creature \u2014 Cat Spirit",
    "rarity": "Common",
    "abilitiesText": [
      "This card enters the battlefield with two +1/+1 counters on it.",
      "If damage would be dealt to This card, prevent that damage. Remove a +1/+1 counter from This card.",
      ""
    ],
    "textbox": [
      "Phantom Tiger enters the battlefield with two +1/+1 counters on it.",
      "If damage would be dealt to Phantom Tiger, prevent that damage. Remove a +1/+1 counter from Phantom Tiger."
    ],
    "power": "1",
    "toughness": "0",
    "converted": 3,
    "abilities": 2
  },
  "Phyrexian Rager": {
    "name": "Phyrexian Rager",
    "color": "Black",
    "cost": "2B",
    "type": "Creature \u2014 Horror",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, you draw a card and you lose 1 life.",
      ""
    ],
    "textbox": [
      "When Phyrexian Rager enters the battlefield, you draw a card and you lose 1 life."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Rakdos Drake": {
    "name": "Rakdos Drake",
    "color": "Black",
    "cost": "2B",
    "type": "Creature \u2014 Drake",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying",
      "Unleash (You may have this creature enter the battlefield with a +1/+1 counter on it. It can\u2019t block as long as it has a +1/+1 counter on it.)"
    ],
    "power": "1",
    "toughness": "2",
    "converted": 3,
    "abilities": 2
  },
  "Riverwheel Aerialists": {
    "name": "Riverwheel Aerialists",
    "color": "Blue",
    "cost": "5U",
    "type": "Creature \u2014 Djinn Monk",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying",
      "Prowess (Whenever you cast a noncreature spell, this creature gets +1/+1 until end of turn.)"
    ],
    "power": "4",
    "toughness": "5",
    "converted": 6,
    "abilities": 2
  },
  "Rotfeaster Maggot": {
    "name": "Rotfeaster Maggot",
    "color": "Black",
    "cost": "4B",
    "type": "Creature \u2014 Insect",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, exile target creature card from a graveyard. You gain life equal to that card\u2019s toughness.",
      ""
    ],
    "textbox": [
      "When Rotfeaster Maggot enters the battlefield, exile target creature card from a graveyard. You gain life equal to that card\u2019s toughness."
    ],
    "power": "3",
    "toughness": "5",
    "converted": 5,
    "abilities": 1
  },
  "Runed Servitor": {
    "name": "Runed Servitor",
    "color": "Artifact",
    "cost": "2",
    "type": "Artifact Creature \u2014 Construct",
    "rarity": "Common",
    "abilitiesText": [
      "When This card dies, each player draws a card.",
      ""
    ],
    "textbox": [
      "When Runed Servitor dies, each player draws a card."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Scion of Ugin": {
    "name": "Scion of Ugin",
    "color": "Colorless",
    "cost": "6",
    "type": "Creature \u2014 Dragon Spirit",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying"
    ],
    "power": "4",
    "toughness": "4",
    "converted": 6,
    "abilities": 1
  },
  "Seeker of the Way": {
    "name": "Seeker of the Way",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Human Warrior",
    "rarity": "Common",
    "abilitiesText": [
      "Whenever you cast a noncreature spell, This card gains lifelink until end of turn.",
      ""
    ],
    "textbox": [
      "Prowess (Whenever you cast a noncreature spell, this creature gets +1/+1 until end of turn.)",
      "Whenever you cast a noncreature spell, Seeker of the Way gains lifelink until end of turn."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 2
  },
  "Shriekgeist": {
    "name": "Shriekgeist",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Spirit",
    "rarity": "Common",
    "abilitiesText": [
      "Whenever This card deals combat damage to a player, that player mills two cards.",
      ""
    ],
    "textbox": [
      "Flying",
      "Whenever Shriekgeist deals combat damage to a player, that player mills two cards."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Splatter Thug": {
    "name": "Splatter Thug",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Human Warrior",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "First strike",
      "Unleash (You may have this creature enter the battlefield with a +1/+1 counter on it. It can\u2019t block as long as it has a +1/+1 counter on it.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 2
  },
  "Stalwart Aven": {
    "name": "Stalwart Aven",
    "color": "White",
    "cost": "2W",
    "type": "Creature \u2014 Bird Soldier",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying (This creature can\u2019t be blocked except by creatures with flying or reach.)",
      "Renown 1 (When this creature deals combat damage to a player, if it isn\u2019t renowned, put a +1/+1 counter on it and it becomes renowned.)"
    ],
    "power": "1",
    "toughness": "3",
    "converted": 3,
    "abilities": 2
  },
  "Student of Ojutai": {
    "name": "Student of Ojutai",
    "color": "White",
    "cost": "3W",
    "type": "Creature \u2014 Human Monk",
    "rarity": "Common",
    "abilitiesText": [
      "Whenever you cast a noncreature spell, you gain 2 life.",
      ""
    ],
    "textbox": [
      "Whenever you cast a noncreature spell, you gain 2 life."
    ],
    "power": "2",
    "toughness": "4",
    "converted": 4,
    "abilities": 1
  },
  "Sustainer of the Realm": {
    "name": "Sustainer of the Realm",
    "color": "White",
    "cost": "2WW",
    "type": "Creature \u2014 Angel",
    "rarity": "Common",
    "abilitiesText": [
      "Whenever This card blocks, it gets +0/+2 until end of turn.",
      ""
    ],
    "textbox": [
      "Flying",
      "Whenever Sustainer of the Realm blocks, it gets +0/+2 until end of turn."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 4,
    "abilities": 2
  },
  "Thrill-Kill Assassin": {
    "name": "Thrill-Kill Assassin",
    "color": "Black",
    "cost": "1B",
    "type": "Creature \u2014 Human Assassin",
    "rarity": "Common",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Deathtouch",
      "Unleash (You may have this creature enter the battlefield with a +1/+1 counter on it. It can\u2019t block as long as it has a +1/+1 counter on it.)"
    ],
    "power": "1",
    "toughness": "2",
    "converted": 2,
    "abilities": 2
  },
  "Timberland Guide": {
    "name": "Timberland Guide",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Human Scout",
    "rarity": "Common",
    "abilitiesText": [
      "When This card enters the battlefield, put a +1/+1 counter on target creature.",
      ""
    ],
    "textbox": [
      "When Timberland Guide enters the battlefield, put a +1/+1 counter on target creature."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Vent Sentinel": {
    "name": "Vent Sentinel",
    "color": "Red",
    "cost": "3R",
    "type": "Creature \u2014 Elemental",
    "rarity": "Common",
    "abilitiesText": [
      "1R, T: This card deals damage to target player or planeswalker equal to the number of creatures with defender you control.",
      ""
    ],
    "textbox": [
      "Defender",
      "1R, T: Vent Sentinel deals damage to target player or planeswalker equal to the number of creatures with defender you control."
    ],
    "power": "2",
    "toughness": "4",
    "converted": 4,
    "abilities": 2
  },
  "Wall of Roots": {
    "name": "Wall of Roots",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Plant Wall",
    "rarity": "Common",
    "abilitiesText": [
      "Put a -0/-1 counter on This card: Add G. Activate only once each turn.",
      ""
    ],
    "textbox": [
      "Defender",
      "Put a -0/-1 counter on Wall of Roots: Add G. Activate only once each turn."
    ],
    "power": "0",
    "toughness": "5",
    "converted": 2,
    "abilities": 2
  },
  "Wight of Precinct Six": {
    "name": "Wight of Precinct Six",
    "color": "Black",
    "cost": "1B",
    "type": "Creature \u2014 Zombie",
    "rarity": "Common",
    "abilitiesText": [
      "This card gets +1/+1 for each creature card in your opponents\u2019 graveyards.",
      ""
    ],
    "textbox": [
      "Wight of Precinct Six gets +1/+1 for each creature card in your opponents\u2019 graveyards."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "keywordsCommon": {
    "abilitiesText": [
      "Flying",
      "Defender",
      "Lifelink",
      "Trample",
      "Unleash",
      "Vigilance",
      "Prowess",
      "Suspend 5\u2014g",
      "Deathtouch",
      "First strike",
      "Suspend 4\u2014r",
      "Suspend 1\u20142gg",
      "Reach",
      "Renown 1"
    ]
  }
}

imaUncommon = {
  "Abzan Battle Priest": {
    "name": "Abzan Battle Priest",
    "color": "White",
    "cost": "3W",
    "type": "Creature \u2014 Human Cleric",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Outlast W (W, T: Put a +1/+1 counter on this creature. Outlast only as a sorcery.)",
      "Each creature you control with a +1/+1 counter on it has lifelink.",
      ""
    ],
    "textbox": [
      "Outlast W (W, T: Put a +1/+1 counter on this creature. Outlast only as a sorcery.)",
      "Each creature you control with a +1/+1 counter on it has lifelink."
    ],
    "power": "3",
    "toughness": "2",
    "converted": 4,
    "abilities": 2
  },
  "Abzan Falconer": {
    "name": "Abzan Falconer",
    "color": "White",
    "cost": "2W",
    "type": "Creature \u2014 Human Soldier",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Outlast W (W, T: Put a +1/+1 counter on this creature. Outlast only as a sorcery.)",
      "Each creature you control with a +1/+1 counter on it has flying.",
      ""
    ],
    "textbox": [
      "Outlast W (W, T: Put a +1/+1 counter on this creature. Outlast only as a sorcery.)",
      "Each creature you control with a +1/+1 counter on it has flying."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 3,
    "abilities": 2
  },
  "Ajani's Pridemate": {
    "name": "Ajani's Pridemate",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Cat Soldier",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Whenever you gain life, put a +1/+1 counter on This card.",
      ""
    ],
    "textbox": [
      "Whenever you gain life, put a +1/+1 counter on Ajani\u2019s Pridemate."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Bladewing's Thrall": {
    "name": "Bladewing's Thrall",
    "color": "Black",
    "cost": "2BB",
    "type": "Creature \u2014 Zombie",
    "rarity": "Uncommon",
    "abilitiesText": [
      "This card has flying as long as you control a Dragon.",
      "When a Dragon enters the battlefield, you may return This card from your graveyard to the battlefield.",
      ""
    ],
    "textbox": [
      "Bladewing\u2019s Thrall has flying as long as you control a Dragon.",
      "When a Dragon enters the battlefield, you may return Bladewing\u2019s Thrall from your graveyard to the battlefield."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 4,
    "abilities": 2
  },
  "Bladewing the Risen": {
    "name": "Bladewing the Risen",
    "color": "BlackRed",
    "cost": "3BBRR",
    "type": "Legendary Creature \u2014 Zombie Dragon",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, you may return target Dragon permanent card from your graveyard to the battlefield.",
      "BR: Dragon creatures get +1/+1 until end of turn.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Bladewing the Risen enters the battlefield, you may return target Dragon permanent card from your graveyard to the battlefield.",
      "BR: Dragon creatures get +1/+1 until end of turn."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 7,
    "abilities": 3
  },
  "Blizzard Specter": {
    "name": "Blizzard Specter",
    "color": "BlueBlack",
    "cost": "2UB",
    "type": "Snow Creature \u2014 Specter",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Whenever This card deals combat damage to a player, choose one \u2014\n\u2022 That player returns a permanent they control to its owner\u2019s hand.\n\u2022 That player discards a card."
    ],
    "textbox": [
      "Flying",
      "Whenever Blizzard Specter deals combat damage to a player, choose one \u2014",
      "\u2022 That player returns a permanent they control to its owner\u2019s hand.",
      "\u2022 That player discards a card."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 4,
    "abilities": 2
  },
  "Bogbrew Witch": {
    "name": "Bogbrew Witch",
    "color": "Black",
    "cost": "3B",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Uncommon",
    "abilitiesText": [
      "2, T: Search your library for a card named Festering Newt or Bubbling Cauldron, put it onto the battlefield tapped, then shuffle.",
      ""
    ],
    "textbox": [
      "2, T: Search your library for a card named Festering Newt or Bubbling Cauldron, put it onto the battlefield tapped, then shuffle."
    ],
    "power": "1",
    "toughness": "3",
    "converted": 4,
    "abilities": 1
  },
  "Burrenton Forge-Tender": {
    "name": "Burrenton Forge-Tender",
    "color": "White",
    "cost": "W",
    "type": "Creature \u2014 Kithkin Wizard",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Protection from red",
      "Sacrifice This card: Prevent all damage a red source of your choice would deal this turn.",
      ""
    ],
    "textbox": [
      "Protection from red",
      "Sacrifice Burrenton Forge-Tender: Prevent all damage a red source of your choice would deal this turn."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 2
  },
  "Carven Caryatid": {
    "name": "Carven Caryatid",
    "color": "Green",
    "cost": "1GG",
    "type": "Creature \u2014 Spirit",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, draw a card.",
      ""
    ],
    "textbox": [
      "Defender (This creature can\u2019t attack.)",
      "When Carven Caryatid enters the battlefield, draw a card."
    ],
    "power": "2",
    "toughness": "5",
    "converted": 3,
    "abilities": 2
  },
  "Cephalid Broker": {
    "name": "Cephalid Broker",
    "color": "Blue",
    "cost": "3U",
    "type": "Creature \u2014 Cephalid",
    "rarity": "Uncommon",
    "abilitiesText": [
      "T: Target player draws two cards, then discards two cards.",
      ""
    ],
    "textbox": [
      "T: Target player draws two cards, then discards two cards."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 4,
    "abilities": 1
  },
  "Chronicler of Heroes": {
    "name": "Chronicler of Heroes",
    "color": "GreenWhite",
    "cost": "1GW",
    "type": "Creature \u2014 Centaur Wizard",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, draw a card if you control a creature with a +1/+1 counter on it.",
      ""
    ],
    "textbox": [
      "When Chronicler of Heroes enters the battlefield, draw a card if you control a creature with a +1/+1 counter on it."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 3,
    "abilities": 1
  },
  "Corpsejack Menace": {
    "name": "Corpsejack Menace",
    "color": "BlackGreen",
    "cost": "2BG",
    "type": "Creature \u2014 Fungus",
    "rarity": "Uncommon",
    "abilitiesText": [
      "If one or more +1/+1 counters would be put on a creature you control, twice that many +1/+1 counters are put on it instead.",
      ""
    ],
    "textbox": [
      "If one or more +1/+1 counters would be put on a creature you control, twice that many +1/+1 counters are put on it instead."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 4,
    "abilities": 1
  },
  "Dragonloft Idol": {
    "name": "Dragonloft Idol",
    "color": "Artifact",
    "cost": "4",
    "type": "Artifact Creature \u2014 Gargoyle",
    "rarity": "Uncommon",
    "abilitiesText": [
      "As long as you control a Dragon, This card gets +1/+1 and has flying and trample.",
      ""
    ],
    "textbox": [
      "As long as you control a Dragon, Dragonloft Idol gets +1/+1 and has flying and trample."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 4,
    "abilities": 1
  },
  "Fog Bank": {
    "name": "Fog Bank",
    "color": "Blue",
    "cost": "1U",
    "type": "Creature \u2014 Wall",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Prevent all combat damage that would be dealt to and dealt by This card.",
      ""
    ],
    "textbox": [
      "Defender, flying",
      "Prevent all combat damage that would be dealt to and dealt by Fog Bank."
    ],
    "power": "0",
    "toughness": "2",
    "converted": 2,
    "abilities": 3
  },
  "Guttersnipe": {
    "name": "Guttersnipe",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Goblin Shaman",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Whenever you cast an instant or sorcery spell, This card deals 2 damage to each opponent.",
      ""
    ],
    "textbox": [
      "Whenever you cast an instant or sorcery spell, Guttersnipe deals 2 damage to each opponent."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Heroes' Bane": {
    "name": "Heroes' Bane",
    "color": "Green",
    "cost": "3GG",
    "type": "Creature \u2014 Hydra",
    "rarity": "Uncommon",
    "abilitiesText": [
      "This card enters the battlefield with four +1/+1 counters on it.",
      "2GG: Put X +1/+1 counters on This card, where X is its power.",
      ""
    ],
    "textbox": [
      "Heroes\u2019 Bane enters the battlefield with four +1/+1 counters on it.",
      "2GG: Put X +1/+1 counters on Heroes\u2019 Bane, where X is its power."
    ],
    "power": "0",
    "toughness": "0",
    "converted": 5,
    "abilities": 2
  },
  "Hoarding Dragon": {
    "name": "Hoarding Dragon",
    "color": "Red",
    "cost": "3RR",
    "type": "Creature \u2014 Dragon",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, you may search your library for an artifact card, exile it, then shuffle.",
      "When This card dies, you may put the exiled card into its owner\u2019s hand.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Hoarding Dragon enters the battlefield, you may search your library for an artifact card, exile it, then shuffle.",
      "When Hoarding Dragon dies, you may put the exiled card into its owner\u2019s hand."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 5,
    "abilities": 3
  },
  "Illusory Ambusher": {
    "name": "Illusory Ambusher",
    "color": "Blue",
    "cost": "4U",
    "type": "Creature \u2014 Cat Illusion",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Whenever This card is dealt damage, draw that many cards.",
      ""
    ],
    "textbox": [
      "Flash (You may cast this spell any time you could cast an instant.)",
      "Whenever Illusory Ambusher is dealt damage, draw that many cards."
    ],
    "power": "4",
    "toughness": "1",
    "converted": 5,
    "abilities": 2
  },
  "Illusory Angel": {
    "name": "Illusory Angel",
    "color": "Blue",
    "cost": "2U",
    "type": "Creature \u2014 Angel Illusion",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Cast this spell only if you\u2019ve cast another spell this turn.",
      ""
    ],
    "textbox": [
      "Cast this spell only if you\u2019ve cast another spell this turn.",
      "Flying"
    ],
    "power": "4",
    "toughness": "4",
    "converted": 3,
    "abilities": 2
  },
  "Indulgent Tormentor": {
    "name": "Indulgent Tormentor",
    "color": "Black",
    "cost": "3BB",
    "type": "Creature \u2014 Demon",
    "rarity": "Uncommon",
    "abilitiesText": [
      "At the beginning of your upkeep, draw a card unless target opponent sacrifices a creature or pays 3 life.",
      ""
    ],
    "textbox": [
      "Flying",
      "At the beginning of your upkeep, draw a card unless target opponent sacrifices a creature or pays 3 life."
    ],
    "power": "5",
    "toughness": "3",
    "converted": 5,
    "abilities": 2
  },
  "Jungle Barrier": {
    "name": "Jungle Barrier",
    "color": "GreenBlue",
    "cost": "2GU",
    "type": "Creature \u2014 Plant Wall",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, draw a card.",
      ""
    ],
    "textbox": [
      "Defender (This creature can\u2019t attack.)",
      "When Jungle Barrier enters the battlefield, draw a card."
    ],
    "power": "2",
    "toughness": "6",
    "converted": 4,
    "abilities": 2
  },
  "Mahamoti Djinn": {
    "name": "Mahamoti Djinn",
    "color": "Blue",
    "cost": "4UU",
    "type": "Creature \u2014 Djinn",
    "rarity": "Uncommon",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying (This creature can\u2019t be blocked except by creatures with flying or reach.)"
    ],
    "power": "5",
    "toughness": "6",
    "converted": 6,
    "abilities": 1
  },
  "Mer-Ek Nightblade": {
    "name": "Mer-Ek Nightblade",
    "color": "Black",
    "cost": "3B",
    "type": "Creature \u2014 Orc Assassin",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Outlast B (B, T: Put a +1/+1 counter on this creature. Outlast only as a sorcery.)",
      "Each creature you control with a +1/+1 counter on it has deathtouch.",
      ""
    ],
    "textbox": [
      "Outlast B (B, T: Put a +1/+1 counter on this creature. Outlast only as a sorcery.)",
      "Each creature you control with a +1/+1 counter on it has deathtouch."
    ],
    "power": "2",
    "toughness": "3",
    "converted": 4,
    "abilities": 2
  },
  "Monastery Swiftspear": {
    "name": "Monastery Swiftspear",
    "color": "Red",
    "cost": "R",
    "type": "Creature \u2014 Human Monk",
    "rarity": "Uncommon",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Haste",
      "Prowess (Whenever you cast a noncreature spell, this creature gets +1/+1 until end of turn.)"
    ],
    "power": "1",
    "toughness": "2",
    "converted": 1,
    "abilities": 2
  },
  "Noxious Dragon": {
    "name": "Noxious Dragon",
    "color": "Black",
    "cost": "4BB",
    "type": "Creature \u2014 Dragon",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card dies, you may destroy target creature with mana value 3 or less.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Noxious Dragon dies, you may destroy target creature with mana value 3 or less."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 6,
    "abilities": 2
  },
  "Overgrown Battlement": {
    "name": "Overgrown Battlement",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Wall",
    "rarity": "Uncommon",
    "abilitiesText": [
      "T: Add G for each creature with defender you control.",
      ""
    ],
    "textbox": [
      "Defender",
      "T: Add G for each creature with defender you control."
    ],
    "power": "0",
    "toughness": "4",
    "converted": 2,
    "abilities": 2
  },
  "Palladium Myr": {
    "name": "Palladium Myr",
    "color": "Artifact",
    "cost": "3",
    "type": "Artifact Creature \u2014 Myr",
    "rarity": "Uncommon",
    "abilitiesText": [
      "T: Add CC.",
      ""
    ],
    "textbox": [
      "T: Add CC."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Prodigal Pyromancer": {
    "name": "Prodigal Pyromancer",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Uncommon",
    "abilitiesText": [
      "T: This card deals 1 damage to any target.",
      ""
    ],
    "textbox": [
      "T: Prodigal Pyromancer deals 1 damage to any target."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 3,
    "abilities": 1
  },
  "Rosheen Meanderer": {
    "name": "Rosheen Meanderer",
    "color": "Gold",
    "cost": "3R/G",
    "type": "Legendary Creature \u2014 Giant Shaman",
    "rarity": "Uncommon",
    "abilitiesText": [
      "T: Add CCCC. Spend this mana only on costs that contain X.",
      ""
    ],
    "textbox": [
      "T: Add CCCC. Spend this mana only on costs that contain X."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 6,
    "abilities": 1
  },
  "Sandstone Oracle": {
    "name": "Sandstone Oracle",
    "color": "Artifact",
    "cost": "7",
    "type": "Artifact Creature \u2014 Sphinx",
    "rarity": "Uncommon",
    "abilitiesText": [
      "When This card enters the battlefield, choose an opponent. If that player has more cards in hand than you, draw cards equal to the difference.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Sandstone Oracle enters the battlefield, choose an opponent. If that player has more cards in hand than you, draw cards equal to the difference."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 7,
    "abilities": 2
  },
  "Serra Angel": {
    "name": "Serra Angel",
    "color": "White",
    "cost": "3WW",
    "type": "Creature \u2014 Angel",
    "rarity": "Uncommon",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying, vigilance"
    ],
    "power": "4",
    "toughness": "4",
    "converted": 5,
    "abilities": 2
  },
  "Sultai Flayer": {
    "name": "Sultai Flayer",
    "color": "Green",
    "cost": "3G",
    "type": "Creature \u2014 Naga Shaman",
    "rarity": "Uncommon",
    "abilitiesText": [
      "Whenever a creature you control with toughness 4 or greater dies, you gain 4 life.",
      ""
    ],
    "textbox": [
      "Whenever a creature you control with toughness 4 or greater dies, you gain 4 life."
    ],
    "power": "3",
    "toughness": "4",
    "converted": 4,
    "abilities": 1
  },
  "Tavern Swindler": {
    "name": "Tavern Swindler",
    "color": "Black",
    "cost": "1B",
    "type": "Creature \u2014 Human Rogue",
    "rarity": "Uncommon",
    "abilitiesText": [
      "T, Pay 3 life: Flip a coin. If you win the flip, you gain 6 life.",
      ""
    ],
    "textbox": [
      "T, Pay 3 life: Flip a coin. If you win the flip, you gain 6 life."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 1
  },
  "Topan Freeblade": {
    "name": "Topan Freeblade",
    "color": "White",
    "cost": "1W",
    "type": "Creature \u2014 Human Soldier",
    "rarity": "Uncommon",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Vigilance (Attacking doesn\u2019t cause this creature to tap.)",
      "Renown 1 (When this creature deals combat damage to a player, if it isn\u2019t renowned, put a +1/+1 counter on it and it becomes renowned.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 2
  },
  "Undercity Troll": {
    "name": "Undercity Troll",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Troll",
    "rarity": "Uncommon",
    "abilitiesText": [
      "2G: Regenerate This card. (The next time this creature would be destroyed this turn, it isn\u2019t. Instead tap it, remove all damage from it, and remove it from combat.)",
      ""
    ],
    "textbox": [
      "Renown 1 (When this creature deals combat damage to a player, if it isn\u2019t renowned, put a +1/+1 counter on it and it becomes renowned.)",
      "2G: Regenerate Undercity Troll. (The next time this creature would be destroyed this turn, it isn\u2019t. Instead tap it, remove all damage from it, and remove it from combat.)"
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 2
  },
  "Vizkopa Guildmage": {
    "name": "Vizkopa Guildmage",
    "color": "WhiteBlack",
    "cost": "WB",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Uncommon",
    "abilitiesText": [
      "1WB: Target creature gains lifelink until end of turn.",
      "1WB: Whenever you gain life this turn, each opponent loses that much life.",
      ""
    ],
    "textbox": [
      "1WB: Target creature gains lifelink until end of turn.",
      "1WB: Whenever you gain life this turn, each opponent loses that much life."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 2,
    "abilities": 2
  },
  "keywordsUncommon": {
    "abilitiesText": [
      "Flying",
      "Defender",
      "Flash",
      "Haste",
      "Prowess",
      "Vigilance",
      "Renown 1"
    ]
  }
}

imaRare = {
  "Abyssal Persecutor": {
    "name": "Abyssal Persecutor",
    "color": "Black",
    "cost": "2BB",
    "type": "Creature \u2014 Demon",
    "rarity": "Rare",
    "abilitiesText": [
      "You can\u2019t win the game and your opponents can\u2019t lose the game.",
      ""
    ],
    "textbox": [
      "Flying, trample",
      "You can\u2019t win the game and your opponents can\u2019t lose the game."
    ],
    "power": "6",
    "toughness": "6",
    "converted": 4,
    "abilities": 3
  },
  "Auriok Champion": {
    "name": "Auriok Champion",
    "color": "White",
    "cost": "WW",
    "type": "Creature \u2014 Human Cleric",
    "rarity": "Rare",
    "abilitiesText": [
      "Protection from black and from red",
      "Whenever another creature enters the battlefield, you may gain 1 life.",
      ""
    ],
    "textbox": [
      "Protection from black and from red",
      "Whenever another creature enters the battlefield, you may gain 1 life."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 2,
    "abilities": 2
  },
  "Blood Baron of Vizkopa": {
    "name": "Blood Baron of Vizkopa",
    "color": "WhiteBlack",
    "cost": "3WB",
    "type": "Creature \u2014 Vampire",
    "rarity": "Rare",
    "abilitiesText": [
      "As long as you have 30 or more life and an opponent has 10 or less life, This card gets +6/+6 and has flying.",
      ""
    ],
    "textbox": [
      "Lifelink, protection from white and from black",
      "As long as you have 30 or more life and an opponent has 10 or less life, Blood Baron of Vizkopa gets +6/+6 and has flying."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 5,
    "abilities": 3
  },
  "Bloodghast": {
    "name": "Bloodghast",
    "color": "Black",
    "cost": "BB",
    "type": "Creature \u2014 Vampire Spirit",
    "rarity": "Rare",
    "abilitiesText": [
      "This card can\u2019t block.",
      "This card has haste as long as an opponent has 10 or less life.",
      "Landfall \u2014 Whenever a land enters the battlefield under your control, you may return This card from your graveyard to the battlefield.",
      ""
    ],
    "textbox": [
      "Bloodghast can\u2019t block.",
      "Bloodghast has haste as long as an opponent has 10 or less life.",
      "Landfall \u2014 Whenever a land enters the battlefield under your control, you may return Bloodghast from your graveyard to the battlefield."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 3
  },
  "Bogardan Hellkite": {
    "name": "Bogardan Hellkite",
    "color": "Red",
    "cost": "6RR",
    "type": "Creature \u2014 Dragon",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card enters the battlefield, it deals 5 damage divided as you choose among any number of targets.",
      ""
    ],
    "textbox": [
      "Flash",
      "Flying",
      "When Bogardan Hellkite enters the battlefield, it deals 5 damage divided as you choose among any number of targets."
    ],
    "power": "5",
    "toughness": "5",
    "converted": 8,
    "abilities": 3
  },
  "Charmbreaker Devils": {
    "name": "Charmbreaker Devils",
    "color": "Red",
    "cost": "5R",
    "type": "Creature \u2014 Devil",
    "rarity": "Rare",
    "abilitiesText": [
      "At the beginning of your upkeep, return an instant or sorcery card at random from your graveyard to your hand.",
      "Whenever you cast an instant or sorcery spell, This card gets +4/+0 until end of turn.",
      ""
    ],
    "textbox": [
      "At the beginning of your upkeep, return an instant or sorcery card at random from your graveyard to your hand.",
      "Whenever you cast an instant or sorcery spell, Charmbreaker Devils gets +4/+0 until end of turn."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 6,
    "abilities": 2
  },
  "Emeria Angel": {
    "name": "Emeria Angel",
    "color": "White",
    "cost": "2WW",
    "type": "Creature \u2014 Angel",
    "rarity": "Rare",
    "abilitiesText": [
      "Landfall \u2014 Whenever a land enters the battlefield under your control, you may create a 1/1 white Bird creature token with flying.",
      ""
    ],
    "textbox": [
      "Flying",
      "Landfall \u2014 Whenever a land enters the battlefield under your control, you may create a 1/1 white Bird creature token with flying."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 4,
    "abilities": 2
  },
  "Firemane Angel": {
    "name": "Firemane Angel",
    "color": "RedWhite",
    "cost": "3RWW",
    "type": "Creature \u2014 Angel",
    "rarity": "Rare",
    "abilitiesText": [
      "At the beginning of your upkeep, if This card is in your graveyard or on the battlefield, you may gain 1 life.",
      "6RRWW: Return This card from your graveyard to the battlefield. Activate only during your upkeep.",
      ""
    ],
    "textbox": [
      "Flying, first strike",
      "At the beginning of your upkeep, if Firemane Angel is in your graveyard or on the battlefield, you may gain 1 life.",
      "6RRWW: Return Firemane Angel from your graveyard to the battlefield. Activate only during your upkeep."
    ],
    "power": "4",
    "toughness": "3",
    "converted": 6,
    "abilities": 4
  },
  "Genesis Hydra": {
    "name": "Genesis Hydra",
    "color": "Green",
    "cost": "XGG",
    "type": "Creature \u2014 Plant Hydra",
    "rarity": "Rare",
    "abilitiesText": [
      "When you cast this spell, reveal the top X cards of your library. You may put a nonland permanent card with mana value X or less from among them onto the battlefield. Then shuffle the rest into your library.",
      "This card enters the battlefield with X +1/+1 counters on it.",
      ""
    ],
    "textbox": [
      "When you cast this spell, reveal the top X cards of your library. You may put a nonland permanent card with mana value X or less from among them onto the battlefield. Then shuffle the rest into your library.",
      "Genesis Hydra enters the battlefield with X +1/+1 counters on it."
    ],
    "power": "0",
    "toughness": "0",
    "converted": 3,
    "abilities": 2
  },
  "Hypersonic Dragon": {
    "name": "Hypersonic Dragon",
    "color": "BlueRed",
    "cost": "3UR",
    "type": "Creature \u2014 Dragon",
    "rarity": "Rare",
    "abilitiesText": [
      "You may cast sorcery spells as though they had flash. (You may cast them any time you could cast an instant.)",
      ""
    ],
    "textbox": [
      "Flying, haste",
      "You may cast sorcery spells as though they had flash. (You may cast them any time you could cast an instant.)"
    ],
    "power": "4",
    "toughness": "4",
    "converted": 5,
    "abilities": 3
  },
  "Jugan, the Rising Star": {
    "name": "Jugan, the Rising Star",
    "color": "Green",
    "cost": "3GGG",
    "type": "Legendary Creature \u2014 Dragon Spirit",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card dies, you may distribute five +1/+1 counters among any number of target creatures.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Jugan, the Rising Star dies, you may distribute five +1/+1 counters among any number of target creatures."
    ],
    "power": "5",
    "toughness": "5",
    "converted": 6,
    "abilities": 2
  },
  "Keiga, the Tide Star": {
    "name": "Keiga, the Tide Star",
    "color": "Blue",
    "cost": "5U",
    "type": "Legendary Creature \u2014 Dragon Spirit",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card dies, gain control of target creature.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Keiga, the Tide Star dies, gain control of target creature."
    ],
    "power": "5",
    "toughness": "5",
    "converted": 6,
    "abilities": 2
  },
  "Knight of the Reliquary": {
    "name": "Knight of the Reliquary",
    "color": "GreenWhite",
    "cost": "1GW",
    "type": "Creature \u2014 Human Knight",
    "rarity": "Rare",
    "abilitiesText": [
      "This card gets +1/+1 for each land card in your graveyard.",
      "T, Sacrifice a Forest or Plains: Search your library for a land card, put it onto the battlefield, then shuffle.",
      ""
    ],
    "textbox": [
      "Knight of the Reliquary gets +1/+1 for each land card in your graveyard.",
      "T, Sacrifice a Forest or Plains: Search your library for a land card, put it onto the battlefield, then shuffle."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 2
  },
  "Kokusho, the Evening Star": {
    "name": "Kokusho, the Evening Star",
    "color": "Black",
    "cost": "4BB",
    "type": "Legendary Creature \u2014 Dragon Spirit",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card dies, each opponent loses 5 life. You gain life equal to the life lost this way.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Kokusho, the Evening Star dies, each opponent loses 5 life. You gain life equal to the life lost this way."
    ],
    "power": "5",
    "toughness": "5",
    "converted": 6,
    "abilities": 2
  },
  "Lord of the Pit": {
    "name": "Lord of the Pit",
    "color": "Black",
    "cost": "4BBB",
    "type": "Creature \u2014 Demon",
    "rarity": "Rare",
    "abilitiesText": [
      "At the beginning of your upkeep, sacrifice a creature other than This card. If you can\u2019t, This card deals 7 damage to you.",
      ""
    ],
    "textbox": [
      "Flying, trample",
      "At the beginning of your upkeep, sacrifice a creature other than Lord of the Pit. If you can\u2019t, Lord of the Pit deals 7 damage to you."
    ],
    "power": "7",
    "toughness": "7",
    "converted": 7,
    "abilities": 3
  },
  "Lotus Cobra": {
    "name": "Lotus Cobra",
    "color": "Green",
    "cost": "1G",
    "type": "Creature \u2014 Snake",
    "rarity": "Rare",
    "abilitiesText": [
      "Landfall \u2014 Whenever a land enters the battlefield under your control, add one mana of any color.",
      ""
    ],
    "textbox": [
      "Landfall \u2014 Whenever a land enters the battlefield under your control, add one mana of any color."
    ],
    "power": "2",
    "toughness": "1",
    "converted": 2,
    "abilities": 1
  },
  "Magus of the Moon": {
    "name": "Magus of the Moon",
    "color": "Red",
    "cost": "2R",
    "type": "Creature \u2014 Human Wizard",
    "rarity": "Rare",
    "abilitiesText": [
      "Nonbasic lands are Mountains.",
      ""
    ],
    "textbox": [
      "Nonbasic lands are Mountains."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 3,
    "abilities": 1
  },
  "Malfegor": {
    "name": "Malfegor",
    "color": "BlackRed",
    "cost": "2BBRR",
    "type": "Legendary Creature \u2014 Demon Dragon",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card enters the battlefield, discard your hand. Each opponent sacrifices a creature for each card discarded this way.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Malfegor enters the battlefield, discard your hand. Each opponent sacrifices a creature for each card discarded this way."
    ],
    "power": "6",
    "toughness": "6",
    "converted": 6,
    "abilities": 2
  },
  "Obstinate Baloth": {
    "name": "Obstinate Baloth",
    "color": "Green",
    "cost": "2GG",
    "type": "Creature \u2014 Beast",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card enters the battlefield, you gain 4 life.",
      "If a spell or ability an opponent controls causes you to discard This card, put it onto the battlefield instead of putting it into your graveyard.",
      ""
    ],
    "textbox": [
      "When Obstinate Baloth enters the battlefield, you gain 4 life.",
      "If a spell or ability an opponent controls causes you to discard Obstinate Baloth, put it onto the battlefield instead of putting it into your graveyard."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 4,
    "abilities": 2
  },
  "Rampaging Baloths": {
    "name": "Rampaging Baloths",
    "color": "Green",
    "cost": "4GG",
    "type": "Creature \u2014 Beast",
    "rarity": "Rare",
    "abilitiesText": [
      "Landfall \u2014 Whenever a land enters the battlefield under your control, you may create a 4/4 green Beast creature token.",
      ""
    ],
    "textbox": [
      "Trample",
      "Landfall \u2014 Whenever a land enters the battlefield under your control, you may create a 4/4 green Beast creature token."
    ],
    "power": "6",
    "toughness": "6",
    "converted": 6,
    "abilities": 2
  },
  "Restoration Angel": {
    "name": "Restoration Angel",
    "color": "White",
    "cost": "3W",
    "type": "Creature \u2014 Angel",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card enters the battlefield, you may exile target non-Angel creature you control, then return that card to the battlefield under your control.",
      ""
    ],
    "textbox": [
      "Flash",
      "Flying",
      "When Restoration Angel enters the battlefield, you may exile target non-Angel creature you control, then return that card to the battlefield under your control."
    ],
    "power": "3",
    "toughness": "4",
    "converted": 4,
    "abilities": 3
  },
  "Rune-Scarred Demon": {
    "name": "Rune-Scarred Demon",
    "color": "Black",
    "cost": "5BB",
    "type": "Creature \u2014 Demon",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card enters the battlefield, search your library for a card, put it into your hand, then shuffle.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Rune-Scarred Demon enters the battlefield, search your library for a card, put it into your hand, then shuffle."
    ],
    "power": "6",
    "toughness": "6",
    "converted": 7,
    "abilities": 2
  },
  "Ryusei, the Falling Star": {
    "name": "Ryusei, the Falling Star",
    "color": "Red",
    "cost": "5R",
    "type": "Legendary Creature \u2014 Dragon Spirit",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card dies, it deals 5 damage to each creature without flying.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Ryusei, the Falling Star dies, it deals 5 damage to each creature without flying."
    ],
    "power": "5",
    "toughness": "5",
    "converted": 6,
    "abilities": 2
  },
  "Savageborn Hydra": {
    "name": "Savageborn Hydra",
    "color": "Gold",
    "cost": "XRG",
    "type": "Creature \u2014 Hydra",
    "rarity": "Rare",
    "abilitiesText": [
      "This card enters the battlefield with X +1/+1 counters on it.",
      "1R/G: Put a +1/+1 counter on This card. Activate only as a sorcery.",
      ""
    ],
    "textbox": [
      "Double strike",
      "Savageborn Hydra enters the battlefield with X +1/+1 counters on it.",
      "1R/G: Put a +1/+1 counter on Savageborn Hydra. Activate only as a sorcery."
    ],
    "power": "0",
    "toughness": "0",
    "converted": 3,
    "abilities": 3
  },
  "Scourge of Valkas": {
    "name": "Scourge of Valkas",
    "color": "Red",
    "cost": "2RRR",
    "type": "Creature \u2014 Dragon",
    "rarity": "Rare",
    "abilitiesText": [
      "Whenever This card or another Dragon enters the battlefield under your control, it deals X damage to any target, where X is the number of Dragons you control.",
      "R: This card gets +1/+0 until end of turn.",
      ""
    ],
    "textbox": [
      "Flying",
      "Whenever Scourge of Valkas or another Dragon enters the battlefield under your control, it deals X damage to any target, where X is the number of Dragons you control.",
      "R: Scourge of Valkas gets +1/+0 until end of turn."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 5,
    "abilities": 3
  },
  "Serra Ascendant": {
    "name": "Serra Ascendant",
    "color": "White",
    "cost": "W",
    "type": "Creature \u2014 Human Monk",
    "rarity": "Rare",
    "abilitiesText": [
      "As long as you have 30 or more life, This card gets +5/+5 and has flying.",
      ""
    ],
    "textbox": [
      "Lifelink (Damage dealt by this creature also causes you to gain that much life.)",
      "As long as you have 30 or more life, Serra Ascendant gets +5/+5 and has flying."
    ],
    "power": "1",
    "toughness": "1",
    "converted": 1,
    "abilities": 2
  },
  "Simic Sky Swallower": {
    "name": "Simic Sky Swallower",
    "color": "GreenBlue",
    "cost": "5GU",
    "type": "Creature \u2014 Leviathan",
    "rarity": "Rare",
    "abilitiesText": [
      ""
    ],
    "textbox": [
      "Flying, trample",
      "Shroud (This creature can\u2019t be the target of spells or abilities.)"
    ],
    "power": "6",
    "toughness": "6",
    "converted": 7,
    "abilities": 3
  },
  "Sphinx of Uthuun": {
    "name": "Sphinx of Uthuun",
    "color": "Blue",
    "cost": "5UU",
    "type": "Creature \u2014 Sphinx",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card enters the battlefield, reveal the top five cards of your library. An opponent separates those cards into two piles. Put one pile into your hand and the other into your graveyard.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Sphinx of Uthuun enters the battlefield, reveal the top five cards of your library. An opponent separates those cards into two piles. Put one pile into your hand and the other into your graveyard."
    ],
    "power": "5",
    "toughness": "6",
    "converted": 7,
    "abilities": 2
  },
  "Spiritmonger": {
    "name": "Spiritmonger",
    "color": "BlackGreen",
    "cost": "3BG",
    "type": "Creature \u2014 Beast",
    "rarity": "Rare",
    "abilitiesText": [
      "Whenever This card deals damage to a creature, put a +1/+1 counter on This card.",
      "B: Regenerate This card.",
      "G: This card becomes the color of your choice until end of turn.",
      ""
    ],
    "textbox": [
      "Whenever Spiritmonger deals damage to a creature, put a +1/+1 counter on Spiritmonger.",
      "B: Regenerate Spiritmonger.",
      "G: Spiritmonger becomes the color of your choice until end of turn."
    ],
    "power": "6",
    "toughness": "6",
    "converted": 5,
    "abilities": 3
  },
  "Teferi, Mage of Zhalfir": {
    "name": "Teferi, Mage of Zhalfir",
    "color": "Blue",
    "cost": "2UUU",
    "type": "Legendary Creature \u2014 Human Wizard",
    "rarity": "Rare",
    "abilitiesText": [
      "Creature cards you own that aren\u2019t on the battlefield have flash.",
      "Each opponent can cast spells only any time they could cast a sorcery.",
      ""
    ],
    "textbox": [
      "Flash",
      "Creature cards you own that aren\u2019t on the battlefield have flash.",
      "Each opponent can cast spells only any time they could cast a sorcery."
    ],
    "power": "3",
    "toughness": "4",
    "converted": 5,
    "abilities": 3
  },
  "Yosei, the Morning Star": {
    "name": "Yosei, the Morning Star",
    "color": "White",
    "cost": "4WW",
    "type": "Legendary Creature \u2014 Dragon Spirit",
    "rarity": "Rare",
    "abilitiesText": [
      "When This card dies, target player skips their next untap step. Tap up to five target permanents that player controls.",
      ""
    ],
    "textbox": [
      "Flying",
      "When Yosei, the Morning Star dies, target player skips their next untap step. Tap up to five target permanents that player controls."
    ],
    "power": "5",
    "toughness": "5",
    "converted": 6,
    "abilities": 2
  },
  "keywordsRare": {
    "abilitiesText": [
      "Flying",
      "Trample",
      "Lifelink",
      "Protection from white and from black",
      "Flash",
      "First strike",
      "Haste",
      "Double strike",
      "Shroud"
    ]
  }
}

imaMythic = {
  "Archangel of Thune": {
    "name": "Archangel of Thune",
    "color": "White",
    "cost": "3WW",
    "type": "Creature \u2014 Angel",
    "rarity": "Mythic",
    "abilitiesText": [
      "Whenever you gain life, put a +1/+1 counter on each creature you control.",
      ""
    ],
    "textbox": [
      "Flying",
      "Lifelink (Damage dealt by this creature also causes you to gain that much life.)",
      "Whenever you gain life, put a +1/+1 counter on each creature you control."
    ],
    "power": "3",
    "toughness": "4",
    "converted": 5,
    "abilities": 3
  },
  "Avacyn, Angel of Hope": {
    "name": "Avacyn, Angel of Hope",
    "color": "White",
    "cost": "5WWW",
    "type": "Legendary Creature \u2014 Angel",
    "rarity": "Mythic",
    "abilitiesText": [
      "Other permanents you control have indestructible.",
      ""
    ],
    "textbox": [
      "Flying, vigilance, indestructible",
      "Other permanents you control have indestructible."
    ],
    "power": "8",
    "toughness": "8",
    "converted": 8,
    "abilities": 4
  },
  "Consecrated Sphinx": {
    "name": "Consecrated Sphinx",
    "color": "Blue",
    "cost": "4UU",
    "type": "Creature \u2014 Sphinx",
    "rarity": "Mythic",
    "abilitiesText": [
      "Whenever an opponent draws a card, you may draw two cards.",
      ""
    ],
    "textbox": [
      "Flying",
      "Whenever an opponent draws a card, you may draw two cards."
    ],
    "power": "4",
    "toughness": "6",
    "converted": 6,
    "abilities": 2
  },
  "Elesh Norn, Grand Cenobite": {
    "name": "Elesh Norn, Grand Cenobite",
    "color": "White",
    "cost": "5WW",
    "type": "Legendary Creature \u2014 Praetor",
    "rarity": "Mythic",
    "abilitiesText": [
      "Other creatures you control get +2/+2.",
      "Creatures your opponents control get -2/-2.",
      ""
    ],
    "textbox": [
      "Vigilance",
      "Other creatures you control get +2/+2.",
      "Creatures your opponents control get -2/-2."
    ],
    "power": "4",
    "toughness": "7",
    "converted": 7,
    "abilities": 3
  },
  "Jin-Gitaxias, Core Augur": {
    "name": "Jin-Gitaxias, Core Augur",
    "color": "Blue",
    "cost": "8UU",
    "type": "Legendary Creature \u2014 Praetor",
    "rarity": "Mythic",
    "abilitiesText": [
      "At the beginning of your end step, draw seven cards.",
      "Each opponent\u2019s maximum hand size is reduced by seven.",
      ""
    ],
    "textbox": [
      "Flash",
      "At the beginning of your end step, draw seven cards.",
      "Each opponent\u2019s maximum hand size is reduced by seven."
    ],
    "power": "5",
    "toughness": "4",
    "converted": 10,
    "abilities": 3
  },
  "Kiki-Jiki, Mirror Breaker": {
    "name": "Kiki-Jiki, Mirror Breaker",
    "color": "Red",
    "cost": "2RRR",
    "type": "Legendary Creature \u2014 Goblin Shaman",
    "rarity": "Mythic",
    "abilitiesText": [
      "T: Create a token that\u2019s a copy of target nonlegendary creature you control, except it has haste. Sacrifice it at the beginning of the next end step.",
      ""
    ],
    "textbox": [
      "Haste",
      "T: Create a token that\u2019s a copy of target nonlegendary creature you control, except it has haste. Sacrifice it at the beginning of the next end step."
    ],
    "power": "2",
    "toughness": "2",
    "converted": 5,
    "abilities": 2
  },
  "Ob Nixilis, the Fallen": {
    "name": "Ob Nixilis, the Fallen",
    "color": "Black",
    "cost": "3BB",
    "type": "Legendary Creature \u2014 Demon",
    "rarity": "Mythic",
    "abilitiesText": [
      "Landfall \u2014 Whenever a land enters the battlefield under your control, you may have target player lose 3 life. If you do, put three +1/+1 counters on This card.",
      ""
    ],
    "textbox": [
      "Landfall \u2014 Whenever a land enters the battlefield under your control, you may have target player lose 3 life. If you do, put three +1/+1 counters on Ob Nixilis, the Fallen."
    ],
    "power": "3",
    "toughness": "3",
    "converted": 5,
    "abilities": 1
  },
  "Primeval Titan": {
    "name": "Primeval Titan",
    "color": "Green",
    "cost": "4GG",
    "type": "Creature \u2014 Giant",
    "rarity": "Mythic",
    "abilitiesText": [
      "Whenever This card enters the battlefield or attacks, you may search your library for up to two land cards, put them onto the battlefield tapped, then shuffle.",
      ""
    ],
    "textbox": [
      "Trample",
      "Whenever Primeval Titan enters the battlefield or attacks, you may search your library for up to two land cards, put them onto the battlefield tapped, then shuffle."
    ],
    "power": "6",
    "toughness": "6",
    "converted": 6,
    "abilities": 2
  },
  "Sheoldred, Whispering One": {
    "name": "Sheoldred, Whispering One",
    "color": "Black",
    "cost": "5BB",
    "type": "Legendary Creature \u2014 Praetor",
    "rarity": "Mythic",
    "abilitiesText": [
      "At the beginning of your upkeep, return target creature card from your graveyard to the battlefield.",
      "At the beginning of each opponent\u2019s upkeep, that player sacrifices a creature.",
      ""
    ],
    "textbox": [
      "Swampwalk (This creature can\u2019t be blocked as long as defending player controls a Swamp.)",
      "At the beginning of your upkeep, return target creature card from your graveyard to the battlefield.",
      "At the beginning of each opponent\u2019s upkeep, that player sacrifices a creature."
    ],
    "power": "6",
    "toughness": "6",
    "converted": 7,
    "abilities": 3
  },
  "Thundermaw Hellkite": {
    "name": "Thundermaw Hellkite",
    "color": "Red",
    "cost": "3RR",
    "type": "Creature \u2014 Dragon",
    "rarity": "Mythic",
    "abilitiesText": [
      "When This card enters the battlefield, it deals 1 damage to each creature with flying your opponents control. Tap those creatures.",
      ""
    ],
    "textbox": [
      "Flying",
      "Haste (This creature can attack and T as soon as it comes under your control.)",
      "When Thundermaw Hellkite enters the battlefield, it deals 1 damage to each creature with flying your opponents control. Tap those creatures."
    ],
    "power": "5",
    "toughness": "5",
    "converted": 5,
    "abilities": 3
  },
  "Urabrask the Hidden": {
    "name": "Urabrask the Hidden",
    "color": "Red",
    "cost": "3RR",
    "type": "Legendary Creature \u2014 Praetor",
    "rarity": "Mythic",
    "abilitiesText": [
      "Creatures you control have haste.",
      "Creatures your opponents control enter the battlefield tapped.",
      ""
    ],
    "textbox": [
      "Creatures you control have haste.",
      "Creatures your opponents control enter the battlefield tapped."
    ],
    "power": "4",
    "toughness": "4",
    "converted": 5,
    "abilities": 2
  },
  "Vorinclex, Voice of Hunger": {
    "name": "Vorinclex, Voice of Hunger",
    "color": "Green",
    "cost": "6GG",
    "type": "Legendary Creature \u2014 Praetor",
    "rarity": "Mythic",
    "abilitiesText": [
      "Whenever you tap a land for mana, add one mana of any type that land produced.",
      "Whenever an opponent taps a land for mana, that land doesn\u2019t untap during its controller\u2019s next untap step.",
      ""
    ],
    "textbox": [
      "Trample",
      "Whenever you tap a land for mana, add one mana of any type that land produced.",
      "Whenever an opponent taps a land for mana, that land doesn\u2019t untap during its controller\u2019s next untap step."
    ],
    "power": "7",
    "toughness": "6",
    "converted": 8,
    "abilities": 3
  },
  "keywordsMythic": {
    "abilitiesText": [
      "Flying",
      "Lifelink",
      "Vigilance",
      "Indestructible",
      "Flash",
      "Haste",
      "Trample",
      "Swampwalk"
    ]
  }
}